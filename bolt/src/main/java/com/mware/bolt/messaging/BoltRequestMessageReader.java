...
/**
 * Reader for Bolt request messages made available via a {@link BigConnectPack.Unpacker}.
 */
public abstract class BoltRequestMessageReader {
    ...
    public void read(BigConnectPack.Unpacker unpacker) throws IOException {
        try {
            doRead(unpacker);
        } catch (BoltIOException e) {
            if (e.causesFailureMessage()) {
                BigConnectError error = BigConnectError.from(e);
    ...
    private void doRead(BigConnectPack.Unpacker unpacker) throws IOException {
        try {
