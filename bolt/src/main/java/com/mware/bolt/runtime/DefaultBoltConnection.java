...
public class DefaultBoltConnection implements BoltConnection {
    private static final BcLogger LOGGER = BcLoggerFactory.getLogger(DefaultBoltConnection.class);
    ...
    public DefaultBoltConnection(BoltChannel channel, PackOutput output, BoltStateMachine machine,
                                 BoltConnectionLifetimeListener listener,
                                 BoltConnectionQueueMonitor queueMonitor) {
        this(channel, output, machine, listener, queueMonitor, DEFAULT_MAX_BATCH_SIZE);
    }
    ...
    protected boolean processNextBatch(int batchCount, boolean exitIfNoJobsAvailable) {
        try {
            ...
            shouldClose.set(true);
            LOGGER.error(String.format("Protocol breach detected in bolt session '%s'.", id()), ex);
        } catch (InterruptedException ex) {
            shouldClose.set(true);
            LOGGER.info("Bolt session '%s' is interrupted probably due to server shutdown.", id());
        } catch (Throwable t) {
            shouldClose.set(true);
            LOGGER.error(String.format("Unexpected error detected in bolt session '%s'.", id()), t);
        } finally {
            if (willClose()) {
                close();
            }
    ...
    public void handleSchedulingError(Throwable t) {
        ...
        if (!willClose()) {
            String message;
            BigConnectError error;
            if (ExceptionUtils.hasCause(t, RejectedExecutionException.class)) {
                error = BigConnectError.from(Status.Request.NoThreadsAvailable, "There are no available threads to serve this request at the moment. You can retry at a later time or consider increasing max thread pool size for bolt connector(s).");
                ...
                error = BigConnectError.fatalFrom(t);
    ...
    private void close() {
        if (closed.compareAndSet(false, true)) {
            try {
                ...
                LOGGER.error(String.format("Unable to close pack output of bolt session '%s'.", id()), t);
            }

            try {
                machine.close();
            } catch (Throwable t) {
                LOGGER.error(String.format("Unable to close state machine of bolt session '%s'.", id()), t);
            } finally {
                notifyDestroyed();
            }
        }
