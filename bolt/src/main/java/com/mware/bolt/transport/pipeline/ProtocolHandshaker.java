...
import com.mware.bolt.transport.BoltProtocolFactory;
import com.mware.bolt.transport.NettyServer;
...
public class ProtocolHandshaker extends ChannelInboundHandlerAdapter {
    ...
    public ProtocolHandshaker(BoltProtocolFactory boltProtocolFactory, BoltChannel boltChannel,
    ...
    public void channelRead(ChannelHandlerContext ctx, Object msg) {
        if (!(msg instanceof ByteBuf)) {
            ...
            LOGGER.warn("Unsupported connection type: 'HTTP'. Bolt protocol only operates over a TCP connection or WebSocket.");
            ctx.close();
            return;
        }
    ...
    public void exceptionCaught(ChannelHandlerContext ctx, Throwable cause) {
        LOGGER.error("Fatal error occurred during protocol handshaking: " + ctx.channel(), cause);
        ctx.close();
    }
    ...
    private boolean verifyBoltPreamble() {
        if (handshakeBuffer.getInt(0) != BOLT_MAGIC_PREAMBLE) {
            LOGGER.debug("Invalid Bolt handshake signature. Expected 0x%08X, but got: 0x%08X", BOLT_MAGIC_PREAMBLE, handshakeBuffer.getInt(0));
            return false;
        }
    ...
    private boolean performHandshake() {
        ...
        if (protocol == null) {
            LOGGER.debug("Failed Bolt handshake: Bolt versions suggested by client '%s' are not supported by this server.", Arrays.toString(suggestions));
        }
