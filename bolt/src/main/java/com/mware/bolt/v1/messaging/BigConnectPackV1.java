...
import com.mware.bolt.v1.packstream.PackType;
...
import static com.mware.bolt.v1.packstream.PackStream.UNKNOWN_SIZE;
import static com.mware.ge.values.storable.Values.byteArray;

/**
 * Extended PackStream packer and unpacker classes for working
 * with BigConnect-specific data types, represented as structures.
 */
public class BigConnectPackV1 implements BigConnectPack {
    ...
    protected static class PackerV1 extends PackStream.Packer implements AnyValueWriter<IOException>, BigConnectPack.Packer {
        private static final int INITIAL_PATH_CAPACITY = 500;
        private static final int NO_SUCH_ID = -1;
        private final Map<TextValue, Integer> nodeIndexes = new HashMap<>(INITIAL_PATH_CAPACITY + 1);
        private final Map<TextValue, Integer> relationshipIndexes = new HashMap<>(INITIAL_PATH_CAPACITY);
        ...
        @Override
        public void writeNode(String nodeId, TextArray labels, MapValue properties, Authorizations authorizations) throws IOException {
            packStructHeader(NODE_SIZE, NODE);
            pack(nodeId);
            packListHeader(labels.length());
            for (int i = 0; i < labels.length(); i++) {
                labels.value(i).writeTo(this, authorizations);
            }
            properties.writeTo(this, authorizations);
        }

        @Override
        public void writeRelationshipReference(String relationshipId) {
            throw new UnsupportedOperationException("Cannot write a raw relationship reference");
        }

        @Override
        public void writeRelationship(String relationshipId, String startNodeId, String endNodeId, TextValue type, MapValue properties, Authorizations authorizations)
                throws IOException {
        ...
        @Override
        public void writePath(NodeValue[] nodes, RelationshipValue[] relationships, Authorizations authorizations) throws IOException {
            ...
            writeNodesForPath(nodes, authorizations);
            writeRelationshipsForPath(relationships, authorizations);
            ...
            for (int i = 1; i <= 2 * relationships.length; i++) {
                if (i % 2 == 0) {
                    ...
                    if (node.id().equals(r.startNode().id())) {
                        pack(index);
                    } else {
                        pack(-index);
                    }
        ...
        private void writeNodesForPath(NodeValue[] nodes, Authorizations authorizations) throws IOException {
            nodeIndexes.clear();
            for (NodeValue node : nodes) {
                nodeIndexes.putIfAbsent(Values.stringValue(node.id()), nodeIndexes.size());
            }
            ...
            if (size > 0) {
                NodeValue node = nodes[0];
                for (TextValue id : nodeIndexes.keySet()) {
                    int i = 1;
                    while (!node.id().equals(id.stringValue())) {
                        node = nodes[i++];
                    }
                    node.writeTo(this, authorizations);
                }
            }
        }

        private void writeRelationshipsForPath(RelationshipValue[] relationships, Authorizations authorizations) throws IOException {
            relationshipIndexes.clear();
            for (RelationshipValue node : relationships) {
                // relationship indexes are one-based
                relationshipIndexes.putIfAbsent(Values.stringValue(node.id()), relationshipIndexes.size() + 1);
            }
            ...
            if (size > 0) {
                {
                    ...
                    for (TextValue id : relationshipIndexes.keySet()) {
                        int i = 1;
                        while (!edge.id().equals(id.stringValue())) {
                        ...
                        edge.properties().writeTo(this, authorizations);
                    }
                }
            }
        }
    ...
    protected static class UnpackerV1 extends PackStream.Unpacker implements BigConnectPack.Unpacker {
