...
import com.mware.ge.cypher.InputPosition;
...
public class CypherAdapterStream implements BoltResult {
    ...
    public CypherAdapterStream(QueryResult result, Clock clock) {
        this.result = result;
        this.clock = clock;
    ...
    protected void addRecordStreamingTime(Visitor visitor, long time) {
        visitor.addMetadata("result_consumed_after", Values.longValue(time));
    }

    @Override
    public String toString() {
        return "CypherAdapterStream{ fieldNames=" + Arrays.toString(fieldNames) + " }";
    }
