 ...
 * storage in Ge.
 */
public class LocalDateTimeBcProperty extends BcProperty<LocalDateTime> {
    public LocalDateTimeBcProperty(String key) {
        super(key);
    }

    @Override
    public Value wrap(LocalDateTime value) {
        return Values.of(value);
    }

    @Override
    public LocalDateTime unwrap(Value value) {
    ...
    public <T extends Element> void updateProperty(
        ...
        LocalDateTime date = newValue == null ? null : LocalDateTime.from(newValue.toInstant());
    ...
    public <T extends Element> void updateProperty(
        ...
        LocalDateTime date = newValue == null ? null : LocalDateTime.from(newValue.toInstant(ZoneOffset.UTC));
        updateProperty(ctx, propertyKey, date, metadata, timestamp);
    }
    ...
    private <T extends Element> boolean isDateNewer(T element, String propertyKey, LocalDateTime newValue) {
        if (element == null) {
            return true;
        }
        LocalDateTime existingValue = getPropertyValue(element, propertyKey);
    ...
    private <T extends Element> boolean isDateOlder(T element, String propertyKey, LocalDateTime newValue) {
        if (element == null) {
            return true;
        }
        LocalDateTime existingValue = getPropertyValue(element, propertyKey);
    ...
    public LocalDateTime getPropertyValueDateTimeUtc(Element element, String propertyKey) {
        return getPropertyValueDateTime(element, propertyKey, ZoneOffset.UTC);
    }


    public LocalDateTime getPropertyValueDateTime(Element element, String propertyKey, ZoneId zoneId) {
        LocalDateTime value = getPropertyValue(element, propertyKey);
        if (value == null) {
            return null;
        }
        return LocalDateTime.ofInstant(value.toInstant(ZoneOffset.UTC), zoneId);
    }

    public LocalDateTime getFirstPropertyValueDateTime(Element element, ZoneId zoneId) {
        LocalDateTime value = getFirstPropertyValue(element);
        if (value == null) {
            return null;
        }
        return LocalDateTime.ofInstant(value.toInstant(ZoneOffset.UTC), zoneId);
