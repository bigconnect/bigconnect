...
public class ElementTracer {
    ...
    private static final Visibility VISIBILITY = new Visibility(GeAuthorizationRepository.ADMIN_ROLE);

    private final Graph graph;
    private final boolean enabled;

    @Inject
    public ElementTracer(Graph graph, Configuration configuration) {
        this.graph = graph;
    ...
    private void ensureTraceVertexExists() {
        if (!this.graph.doesVertexExist(TRACER_VERTEX_ID, AUTHORIZATIONS_ALL)) {
            this.graph
    ...
    public void trace(String elementId, String action, String description) {
        ...
        this.graph.getVertex(TRACER_VERTEX_ID, AUTHORIZATIONS_ALL).prepareMutation()
    ...
    public List<ElementTraceInfo> getTraces(String elementId) {
        try (QueryResultsIterable<ExtendedDataRow> rows = this.graph.getVertex(TRACER_VERTEX_ID, AUTHORIZATIONS_ALL).getExtendedData(TRACER_TABLE)
                .query(AUTHORIZATIONS_ALL)
