...
import static com.mware.ge.csv.Mark.END_OF_LINE_CHARACTER;
    ...
    public boolean seek(Mark mark, int untilChar) throws IOException {
        ...
        while (!eof) {
            ...
            if (quoteDepth == 0) {   // In normal mode, i.e. not within quotes
                ...
                } else if (trim && isWhitespace(ch)) {   // Only check for left+trim whitespace as long as we haven't found a non-whitespace character
                    if (seekStartPos == bufferPos - 1/* -1 since we just advanced one */) {   // We found a whitespace, which is before the first non-whitespace of the value and we've been told to trim that off
                        seekStartPos++;
                    }
                } else if (ch == quoteChar && seekStartPos == bufferPos - 1/* -1 since we just advanced one */) {   // We found a quote, which was the first of the value, skip it and switch mode
                    quoteDepth++;
                    isQuoted = true;
                    seekStartPos++;
                ...
                } else if (ch == BACK_SLASH && legacyStyleQuoting) {   // Legacy concern, support java style quote encoding
                    int nextCh = peekChar(skippedChars);
