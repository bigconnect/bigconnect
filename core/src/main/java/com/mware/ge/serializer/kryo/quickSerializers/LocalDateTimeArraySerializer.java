...
class LocalDateTimeArraySerializer implements QuickTypeSerializer<LocalDateTimeArray> {
    ...
    public byte[] objectToBytes(LocalDateTimeArray value) {
        LocalDateTime[] v = value.asObjectCopy();
        int arrLength = v.length;
        ByteBuffer buf = ByteBuffer.allocate(1 + Integer.BYTES + 2 * Long.BYTES * arrLength).order(ByteOrder.BIG_ENDIAN);
        buf.put(MARKER_LOCALDATETIMEARRAY);
        buf.putInt(arrLength);
        for (int i = 0; i < value.length(); i++) {
            long epochSecond = v[i].toEpochSecond( UTC );
