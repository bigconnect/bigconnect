...
class LongArraySerializer implements QuickTypeSerializer<LongArray> {
    @Override
    public byte[] objectToBytes(LongArray value) {
        ByteBuffer buf = ByteBuffer.allocate(1 + Long.BYTES * value.length())
                .order(ByteOrder.BIG_ENDIAN)
                .put(MARKER_LONGARRAY);
        buf.asLongBuffer().put(value.asObjectCopy());
        return buf.array();
    }

    public static byte[] objectToBytesNoMarker(long[] value) {
        byte[] bytes = new byte[8 * value.length]; // long has 8 bytes
