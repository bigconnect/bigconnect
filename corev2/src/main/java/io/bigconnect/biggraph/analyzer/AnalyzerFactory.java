...
package io.bigconnect.biggraph.analyzer;
...
public class AnalyzerFactory {
    ...
    private static Analyzer customizedAnalyzer(String name, String mode) {
        ...
        if (clazz == null) {
            throw new BigGraphException("Not exists analyzer: %s", name);
        }
    ...
    public static void register(String name, String classPath) {
        ...
        try {
            ...
            throw new BigGraphException("Load class path '%s' failed",
                    e, classPath);
        }

        // Check subclass
        if (!Analyzer.class.isAssignableFrom(clazz)) {
            throw new BigGraphException("Class '%s' is not a subclass of " +
                    "class Analyzer", classPath);
        }

        // Check exists
        if (analyzers.containsKey(name)) {
            throw new BigGraphException("Exists analyzer: %s(%s)",
