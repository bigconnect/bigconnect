...
public class RolePermission {
    ...
    @JsonProperty("roles")
    private final Map<String, Map<BigPermission, List<BigResource>>> roles;

    public RolePermission() {
        this(new TreeMap<>());
    }

    private RolePermission(Map<String, Map<BigPermission,
                                       List<BigResource>>> roles) {
        this.roles = roles;
    }

    protected void add(String graph, String action,
                       List<BigResource> resources) {
        this.add(graph, BigPermission.valueOf(action), resources);
    }

    protected void add(String graph, BigPermission action,
                       List<BigResource> resources) {
        Map<BigPermission, List<BigResource>> permissions =
                                                this.roles.get(graph);
        if (permissions == null) {
            permissions = new TreeMap<>();
            this.roles.put(graph, permissions);
        }
        List<BigResource> mergedResources = permissions.get(action);
    ...
    protected Map<String, Map<BigPermission, List<BigResource>>> map() {
        return Collections.unmodifiableMap(this.roles);
    }

    protected boolean contains(RolePermission other) {
        for (Map.Entry<String, Map<BigPermission, List<BigResource>>> e1 :
             other.roles.entrySet()) {
            String g = e1.getKey();
            Map<BigPermission, List<BigResource>> perms = this.roles.get(g);
            if (perms == null) {
                return false;
            }
            for (Map.Entry<BigPermission, List<BigResource>> e2 :
                e1.getValue().entrySet()) {
                List<BigResource> ress = perms.get(e2.getKey());
                if (ress == null) {
                    return false;
                }
                for (BigResource r : e2.getValue()) {
                    boolean contains = false;
                    for (BigResource res : ress) {
                        if (res.contains(r)) {
                            contains = true;
                            break;
                        }
    ...
    public static RolePermission all(String graph) {
        return role(graph, BigPermission.ANY);
    }

    public static RolePermission role(String graph, BigPermission perm) {
        RolePermission role = new RolePermission();
        role.add(graph, perm, BigResource.ALL_RES);
        return role;
    }
    ...
    private static class RolePermissionDeser
        ...
        public RolePermission deserialize(JsonParser parser,
            ...
            TypeReference<?> type = new TypeReference<TreeMap<String,
                             TreeMap<BigPermission, List<BigResource>>>>() {};
