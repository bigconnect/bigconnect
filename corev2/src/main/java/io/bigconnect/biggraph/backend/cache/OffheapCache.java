...
import io.bigconnect.biggraph.backend.serializer.BinaryBackendEntry;
...
public class OffheapCache extends AbstractCache<Id, Object> {
    ...
    public OffheapCache(BigGraph graph, long capacity, long avgEntryBytes) {
    ...
    private class Value {
        ...
        private void serializeElement(BytesBuffer buffer,
            ...
            if (type == ValueType.VERTEX) {
                entry = serializer().writeVertex((BigVertex) value);
            } else if (type == ValueType.EDGE) {
                entry = serializer().writeEdge((BigEdge) value);
            } else {
                throw unsupported(type);
            }
        ...
        private Object deserializeElement(ValueType type, BytesBuffer buffer) {
            ...
            if (type == ValueType.VERTEX) {
                entry = new BinaryBackendEntry(BigType.VERTEX, key);
                entry.column(key, value);
                return serializer().readVertex(graph(), entry);
            } else if (type == ValueType.EDGE) {
                entry = new BinaryBackendEntry(BigType.EDGE, key);
                entry.column(key, value);
        ...
        private BigGraphException unsupported(ValueType type) {
            throw new BigGraphException(
                      "Unsupported deserialize type: %s", type);
        }

        private BigGraphException unsupported(Object value) {
            throw new BigGraphException(
    ...
    private static enum ValueType {
        ...
        public static ValueType valueOf(Object object) {
            ...
            } else if (clazz == BigVertex.class) {
                return ValueType.VERTEX;
            } else if (clazz == BigEdge.class) {
