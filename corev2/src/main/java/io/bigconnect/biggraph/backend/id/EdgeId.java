...
public class EdgeId implements Id {
    ...
    public static final BigKeys[] KEYS = new BigKeys[] {
            BigKeys.OWNER_VERTEX,
            BigKeys.DIRECTION,
            BigKeys.LABEL,
            BigKeys.SORT_VALUES,
            BigKeys.OTHER_VERTEX
    };
    ...
    public EdgeId(BigVertex ownerVertex, Directions direction,
                  Id edgeLabelId, String sortValues, BigVertex otherVertex) {
    ...
    public static Directions directionFromCode(byte code) {
        return Directions.convert(BigType.fromCode(code));
    }

    public static boolean isOutDirectionFromCode(byte code) {
        return code == BigType.EDGE_OUT.code();
    }
    ...
    public static EdgeId parse(String id, boolean returnNullIfError)
        ...
        try {
            if (idParts.length == 4) {
                ...
                Id ownerVertexId = IdUtil.readString(idParts[0]);
                BigType direction = BigType.fromString(idParts[1]);
