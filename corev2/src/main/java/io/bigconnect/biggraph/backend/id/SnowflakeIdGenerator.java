...
package io.bigconnect.biggraph.backend.id;
...
import io.bigconnect.biggraph.util.TimeUtil;
...
public class SnowflakeIdGenerator extends IdGenerator {
    ...
    public static SnowflakeIdGenerator init(BigGraphParams graph) {
    ...
    public static SnowflakeIdGenerator instance(BigGraph graph) {
        String graphName = graph.name();
        SnowflakeIdGenerator generator = INSTANCES.get(graphName);
    ...
    private SnowflakeIdGenerator(BigConfig config) {
    ...
    public Id generate() {
        if (this.idWorker == null) {
            throw new BigGraphException("Please initialize before using");
        }
    ...
    @Override
    public Id generate(BigVertex vertex) {
        ...
        public synchronized long nextId() {
            ...
            if (timestamp > this.lastTimestamp) {
                ...
                LOG.error("Clock is moving backwards, " +
                          ...
                          this.lastTimestamp);
                throw new BigGraphException("Clock moved backwards. Refusing to " +
