...
public final class ConditionQueryFlatten {
    ...
    private static final Set<BigKeys> SPECIAL_KEYS = ImmutableSet.of(
            BigKeys.LABEL
    );
    ...
    private static Condition convIn2Or(Relation relation) {
        ...
        for (Object value : values) {
            if (key instanceof BigKeys) {
                cond = Condition.eq((BigKeys) key, value);
            } else {
    ...
    private static Condition convNotin2And(Relation relation) {
        ...
        for (Object value : values) {
            if (key instanceof BigKeys) {
                cond = Condition.neq((BigKeys) key, value);
            } else {
