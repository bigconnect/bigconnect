...
public class Query implements Cloneable {
    ...
    protected static final Query NONE = new Query(BigType.UNKNOWN);

    private static final Set<Id> EMPTY_OLAP_PKS = ImmutableSet.of();

    private BigType resultType;
    private Map<BigKeys, Order> orders;
    private long offset;
    ...
    public Query(BigType resultType) {
        this(resultType, null);
    }

    public Query(BigType resultType, Query originQuery) {
    ...
    public BigType resultType() {
        return this.resultType;
    }

    public void resultType(BigType resultType) {
        this.resultType = resultType;
    }
    ...
    public Map<BigKeys, Order> orders() {
        return Collections.unmodifiableMap(this.getOrNewOrders());
    }

    public void orders(Map<BigKeys, Order> orders) {
        this.orders = InsertionOrderUtil.newMap(orders);
    }

    public void order(BigKeys key, Order order) {
        this.getOrNewOrders().put(key, order);
    }

    protected Map<BigKeys, Order> getOrNewOrders() {
        if (this.orders != null) {
            return this.orders;
        }
