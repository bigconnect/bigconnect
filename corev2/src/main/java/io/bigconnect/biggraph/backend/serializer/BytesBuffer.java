...
public final class BytesBuffer extends OutputStream {
    ...
    public BytesBuffer writeIndexId(Id id, BigType type) {
        return this.writeIndexId(id, type, true);
    }

    public BytesBuffer writeIndexId(Id id, BigType type, boolean withEnding) {
        byte[] bytes = id.asBytes();
        int len = bytes.length;
        E.checkArgument(len > 0, "Can't write empty id");
    ...
    public BinaryId readIndexId(BigType type) {
        byte[] id;
    ...
    public BinaryId parseId(BigType type) {
        if (type.isIndex()) {
