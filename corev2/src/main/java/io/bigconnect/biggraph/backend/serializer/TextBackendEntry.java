...
public class TextBackendEntry implements BackendEntry, Cloneable {
    ...
    private final BigType type;
    private final Id id;
    private Id subId;
    private NavigableMap<String, String> columns;

    public TextBackendEntry(BigType type, Id id) {
        this.type = type;
        this.id = id;
        this.subId = null;
        this.resetColumns();
    ...
    public void column(BigKeys column, String value) {
    ...
    public String column(BigKeys column) {
    ...
    public void append(TextBackendEntry entry) {
        for (Entry<String, String> col : entry.columns.entrySet()) {
            ...
            if (col.getKey().startsWith(BigType.PROPERTY.string())) {
                this.columns.put(col.getKey(), col.getValue());
                continue;
            }

            // TODO: use more general method
            if (!col.getKey().endsWith(BigKeys.ELEMENT_IDS.string())) {
                continue;
            }
    ...
    public void eliminate(TextBackendEntry entry) {
        for (Entry<String, String> col : entry.columns.entrySet()) {
            ...
            // TODO: use more general method
            if (col.getKey().startsWith(BigType.PROPERTY.string()) ||
                col.getKey().startsWith(BigType.EDGE_OUT.string()) ||
                col.getKey().startsWith(BigType.EDGE_IN.string())) {
    ...
    public boolean equals(Object obj) {
        ...
        for (Entry<String, String> e : this.columns.entrySet()) {
            String key = e.getKey();
            String value = e.getValue();
