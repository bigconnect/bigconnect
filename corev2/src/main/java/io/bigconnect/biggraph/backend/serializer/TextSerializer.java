...
import io.bigconnect.biggraph.backend.id.*;
...
import io.bigconnect.biggraph.backend.tx.GraphIndexTransaction;
import io.bigconnect.biggraph.schema.*;
import io.bigconnect.biggraph.structure.*;
import io.bigconnect.biggraph.structure.BigIndex.IdWithExpiredTime;
...
public class TextSerializer extends AbstractSerializer {
    ...
    private static final String EDGE_OUT_TYPE = writeType(BigType.EDGE_OUT);

    @Override
    public TextBackendEntry newBackendEntry(BigType type, Id id) {
        return new TextBackendEntry(type, id);
    }

    private TextBackendEntry newBackendEntry(BigElement elem) {
    ...
    protected TextBackendEntry convertEntry(BackendEntry backendEntry) {
        if (!(backendEntry instanceof TextBackendEntry)) {
            throw new BigGraphException("The entry '%s' is not TextBackendEntry",
                                    backendEntry);
        }
    ...
    private String formatSyspropName(String name) {
        return SplicingIdGenerator.concat(writeType(BigType.SYS_PROPERTY),
                                          name);
    }

    private String formatSyspropName(BigKeys col) {
        return this.formatSyspropName(col.string());
    }

    private String formatPropertyName(String key) {
        return SplicingIdGenerator.concat(writeType(BigType.PROPERTY), key);
    }

    private String formatPropertyName(BigProperty<?> prop) {
        return this.formatPropertyName(writeId(prop.propertyKey().id()));
    }

    private String formatPropertyValue(BigProperty<?> prop) {
        // May be a single value or a list of values
        return JsonUtil.toJson(prop.value());
    }

    private String formatPropertyName() {
        return BigType.PROPERTY.string();
    }

    private String formatPropertyValues(BigVertex vertex) {
        int size = vertex.getProperties().size();
        StringBuilder sb = new StringBuilder(64 * size);
        // Vertex properties
        int i = 0;
        for (BigProperty<?> property : vertex.getProperties().values()) {
    ...
    private void parseProperties(String colValue, BigVertex vertex) {
    ...
    private void parseEdge(String colName, String colValue,
        ...
        String name = this.formatSyspropName(BigKeys.EXPIRED_TIME);
    ...
    private void parseColumn(String colName, String colValue,
        ...
        // Parse property
        if (type.equals(writeType(BigType.PROPERTY))) {
            this.parseProperties(colValue, vertex);
        }
        // Parse edge
        else if (type.equals(writeType(BigType.EDGE_OUT)) ||
                 type.equals(writeType(BigType.EDGE_IN))) {
            this.parseEdge(colName, colValue, vertex);
        }
        // Parse system property
        else if (type.equals(writeType(BigType.SYS_PROPERTY))) {
            // pass
        }
        // Invalid entry
        else {
    ...
    public BackendEntry writeVertex(BigVertex vertex) {
        ...
        if (vertex.schemaLabel() != null) {
            entry.column(this.formatSyspropName(BigKeys.LABEL),
                         writeId(vertex.schemaLabel().id()));
        }

        // Write expired time
        entry.column(this.formatSyspropName(BigKeys.EXPIRED_TIME),
    ...
    public BigVertex readVertex(BigGraph graph, BackendEntry backendEntry) {
        ...
        // Parse label
        String labelId = entry.column(this.formatSyspropName(BigKeys.LABEL));
    ...
    public BackendEntry writeIndex(BigIndex index) {
        ...
        if (index.fieldValues() == null && index.elementIds().size() == 0) {
             ...
             */
            entry.column(BigKeys.INDEX_LABEL_ID,
                         writeId(index.indexLabelId()));
        } else {
            // TODO: field-values may be a number (range index)
            entry.column(formatSyspropName(BigKeys.FIELD_VALUES),
                         JsonUtil.toJson(index.fieldValues()));
            entry.column(formatSyspropName(BigKeys.INDEX_LABEL_ID),
                         writeId(index.indexLabelId()));
            entry.column(formatSyspropName(BigKeys.ELEMENT_IDS),
    ...
    public BigIndex readIndex(BigGraph graph, ConditionQuery query,
        ...
        String indexValues = entry.column(
                             formatSyspropName(BigKeys.FIELD_VALUES));
        String indexLabelId = entry.column(
                              formatSyspropName(BigKeys.INDEX_LABEL_ID));
        String elemIds = entry.column(
                         formatSyspropName(BigKeys.ELEMENT_IDS));
    ...
    @Override
    public TextBackendEntry writeId(BigType type, Id id) {
    ...
    private Query writeQueryEdgeRangeCondition(ConditionQuery cq) {
        ...
        Object vertex = cq.condition(BigKeys.OWNER_VERTEX);
        Object direction = cq.condition(BigKeys.DIRECTION);
        if (direction == null) {
            direction = Directions.OUT;
        }
        Object label = cq.condition(BigKeys.LABEL);
    ...
    private Query writeQueryEdgePrefixCondition(ConditionQuery cq) {
        ...
        for (BigKeys key : EdgeId.KEYS) {
            ...
            if (key == BigKeys.OWNER_VERTEX || key == BigKeys.OTHER_VERTEX) {
    ...
    protected Query writeQueryCondition(Query query) {
        ...
        for (Condition.Relation r : result.relations()) {
            ...
            if (query.resultType().isSchema()) {
                r.serialKey(((BigKeys) r.key()).string());
            } else {
                r.serialKey(formatSyspropName((BigKeys) r.key()));
            }
    ...
    public BackendEntry writeVertexLabel(VertexLabel vertexLabel) {
        ...
        entry.column(BigKeys.NAME, JsonUtil.toJson(vertexLabel.name()));
        entry.column(BigKeys.ID_STRATEGY,
    ...
    public VertexLabel readVertexLabel(BigGraph graph,
        ...
        String name = JsonUtil.fromJson(entry.column(BigKeys.NAME),
                                        String.class);
        String idStrategy = entry.column(BigKeys.ID_STRATEGY);
        String properties = entry.column(BigKeys.PROPERTIES);
        String primaryKeys = entry.column(BigKeys.PRIMARY_KEYS);
        String nullableKeys = entry.column(BigKeys.NULLABLE_KEYS);
        String indexLabels = entry.column(BigKeys.INDEX_LABELS);
        String enableLabelIndex = entry.column(BigKeys.ENABLE_LABEL_INDEX);
        String status = entry.column(BigKeys.STATUS);
    ...
    public BackendEntry writeEdgeLabel(EdgeLabel edgeLabel) {
        ...
        entry.column(BigKeys.NAME, JsonUtil.toJson(edgeLabel.name()));
        entry.column(BigKeys.SOURCE_LABEL, writeId(edgeLabel.sourceLabel()));
        entry.column(BigKeys.TARGET_LABEL, writeId(edgeLabel.targetLabel()));
        entry.column(BigKeys.FREQUENCY,
                     JsonUtil.toJson(edgeLabel.frequency()));
        entry.column(BigKeys.PROPERTIES, writeIds(edgeLabel.properties()));
        entry.column(BigKeys.SORT_KEYS, writeIds(edgeLabel.sortKeys()));
        entry.column(BigKeys.NULLABLE_KEYS,
                     writeIds(edgeLabel.nullableKeys()));
        entry.column(BigKeys.INDEX_LABELS, writeIds(edgeLabel.indexLabels()));
        entry.column(BigKeys.ENABLE_LABEL_INDEX,
                     JsonUtil.toJson(edgeLabel.enableLabelIndex()));
        writeUserdata(edgeLabel, entry);
        entry.column(BigKeys.STATUS,
                     JsonUtil.toJson(edgeLabel.status()));
        entry.column(BigKeys.TTL, JsonUtil.toJson(edgeLabel.ttl()));
    ...
    public EdgeLabel readEdgeLabel(BigGraph graph,
        ...
        String name = JsonUtil.fromJson(entry.column(BigKeys.NAME),
                                        String.class);
        String sourceLabel = entry.column(BigKeys.SOURCE_LABEL);
        String targetLabel = entry.column(BigKeys.TARGET_LABEL);
        String frequency = entry.column(BigKeys.FREQUENCY);
        String sortKeys = entry.column(BigKeys.SORT_KEYS);
        String nullablekeys = entry.column(BigKeys.NULLABLE_KEYS);
        String properties = entry.column(BigKeys.PROPERTIES);
        String indexLabels = entry.column(BigKeys.INDEX_LABELS);
        String enableLabelIndex = entry.column(BigKeys.ENABLE_LABEL_INDEX);
        String status = entry.column(BigKeys.STATUS);
        String ttl = entry.column(BigKeys.TTL);
        String ttlStartTime = entry.column(BigKeys.TTL_START_TIME);
    ...
    public BackendEntry writePropertyKey(PropertyKey propertyKey) {
        ...
        entry.column(BigKeys.NAME, JsonUtil.toJson(propertyKey.name()));
        entry.column(BigKeys.DATA_TYPE,
                     JsonUtil.toJson(propertyKey.dataType()));
        entry.column(BigKeys.CARDINALITY,
        ...
        entry.column(BigKeys.PROPERTIES, writeIds(propertyKey.properties()));
    ...
    public PropertyKey readPropertyKey(BigGraph graph,
        ...
        String name = JsonUtil.fromJson(entry.column(BigKeys.NAME),
                                        String.class);
        String dataType = entry.column(BigKeys.DATA_TYPE);
        String cardinality = entry.column(BigKeys.CARDINALITY);
        String aggregateType = entry.column(BigKeys.AGGREGATE_TYPE);
        String writeType = entry.column(BigKeys.WRITE_TYPE);
        String properties = entry.column(BigKeys.PROPERTIES);
        String status = entry.column(BigKeys.STATUS);
    ...
    public BackendEntry writeIndexLabel(IndexLabel indexLabel) {
        ...
        entry.column(BigKeys.NAME, JsonUtil.toJson(indexLabel.name()));
        entry.column(BigKeys.BASE_TYPE,
                     JsonUtil.toJson(indexLabel.baseType()));
        entry.column(BigKeys.BASE_VALUE, writeId(indexLabel.baseValue()));
        entry.column(BigKeys.INDEX_TYPE,
                     JsonUtil.toJson(indexLabel.indexType()));
        entry.column(BigKeys.FIELDS, writeIds(indexLabel.indexFields()));
    ...
    public IndexLabel readIndexLabel(BigGraph graph,
        ...
        String name = JsonUtil.fromJson(entry.column(BigKeys.NAME),
                                        String.class);
        String baseType = entry.column(BigKeys.BASE_TYPE);
        String baseValue = entry.column(BigKeys.BASE_VALUE);
        String indexType = entry.column(BigKeys.INDEX_TYPE);
        String indexFields = entry.column(BigKeys.FIELDS);
        String status = entry.column(BigKeys.STATUS);

        IndexLabel indexLabel = new IndexLabel(graph, id, name);
        indexLabel.baseType(JsonUtil.fromJson(baseType, BigType.class));
    ...
    private static String writeType(BigType type) {
        return type.string();
    }
    ...
    private static String writeElementId(Id id, long expiredTime) {
        ...
        if (expiredTime <= 0L) {
            ...
            array[0] = ImmutableMap.of(BigKeys.ID.string(), idValue,
                                       BigKeys.EXPIRED_TIME.string(),
                                       expiredTime);
        }
        return JsonUtil.toJson(array);
    }
    ...
    private static IdWithExpiredTime[] readElementIds(String str) {
        ...
        for (int i = 0; i < values.length; i++) {
            ...
            if (values[i] instanceof Map) {
                ...
                idValue = map.get(BigKeys.ID.string());
                expiredTime = ((Number) map.get(
                              BigKeys.EXPIRED_TIME.string())).longValue();
            } else {
                idValue = values[i];
                expiredTime = 0L;
            }
    ...
    private static void writeUserdata(SchemaElement schema,
        ...
        entry.column(BigKeys.USER_DATA, JsonUtil.toJson(schema.userdata()));
    }

    private static void readUserdata(SchemaElement schema,
                                     TextBackendEntry entry) {
        // Parse all user data of a schema element
        String userdataStr = entry.column(BigKeys.USER_DATA);
        @SuppressWarnings("unchecked")
