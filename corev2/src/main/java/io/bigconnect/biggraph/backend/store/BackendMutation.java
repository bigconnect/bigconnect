...
public class BackendMutation {
    ...
    private static BigGraphException incompatibleActionException(
                                 Action newAction,
                                 Action originAction) {
        return new BigGraphException("The action '%s' is incompatible with " +
     ...
     * @return mutations
     */
    public Iterator<BackendAction> mutation(BigType type) {
     ...
     * @return mutations
     */
    public List<BackendAction> mutation(BigType type, Id id) {
     ...
     * @return true if exist, otherwise false
     */
    public boolean contains(BigType type, Action action) {
    ...
    private static class MutationTable {
        ...
        private final Map<BigType, Map<Id, List<BackendAction>>> mutations;
        ...
        public void put(BigType type, Id id, BackendAction mutation) {
        ...
        public boolean containsKey(BigType type, Id id) {
            Map<Id, List<BackendAction>> table = this.mutations.get(type);
            return table != null && table.containsKey(id);
        }

        public List<BackendAction> get(BigType type, Id id) {
            Map<Id, List<BackendAction>> table = this.mutations.get(type);
            if (table == null) {
                return null;
            }
        ...
        public Iterator<BackendAction> get(BigType type) {
