...
import io.bigconnect.biggraph.backend.store.memory.InMemoryDBStoreProvider;
...
public class BackendProviderFactory {
    ...
    public static BackendStoreProvider open(BigGraphParams params) {
    ...
    private static BackendStoreProvider newProvider(BigConfig config) {
        String backend = config.get(CoreOptions.BACKEND).toLowerCase();
        String graph = config.get(CoreOptions.STORE);
