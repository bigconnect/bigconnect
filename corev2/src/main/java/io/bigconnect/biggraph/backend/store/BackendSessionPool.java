...
public abstract class BackendSessionPool {
    ...
    private final BigConfig config;
    private final String name;
    private final ThreadLocal<io.bigconnect.biggraph.backend.store.BackendSession> threadLocalSession;
    private final AtomicInteger sessionCount;
    private final Map<Long, io.bigconnect.biggraph.backend.store.BackendSession> sessions;
    private final long reconnectDetectInterval;

    public BackendSessionPool(BigConfig config, String name) {
        this.config = config;
        this.name = name;
    ...
    public final io.bigconnect.biggraph.backend.store.BackendSession getOrNewSession() {
        io.bigconnect.biggraph.backend.store.BackendSession session = this.threadLocalSession.get();
        if (session == null) {
    ...
    public io.bigconnect.biggraph.backend.store.BackendSession useSession() {
        io.bigconnect.biggraph.backend.store.BackendSession session = this.threadLocalSession.get();
        if (session != null) {
            session.attach();
            this.detectSession(session);
    ...
    private void detectSession(io.bigconnect.biggraph.backend.store.BackendSession session) {
    ...
    public void forceResetSessions() {
        for (io.bigconnect.biggraph.backend.store.BackendSession session : this.sessions.values()) {
            session.reset();
        }
    }
    ...
    public abstract io.bigconnect.biggraph.backend.store.BackendSession session();

    protected abstract io.bigconnect.biggraph.backend.store.BackendSession newSession();
