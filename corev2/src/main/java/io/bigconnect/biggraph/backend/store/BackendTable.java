...
public abstract class BackendTable<Session extends BackendSession, Entry> {
    ...
    public io.bigconnect.biggraph.backend.store.MetaDispatcher<Session> metaDispatcher() {
    ...
    public static BigType tableType(Query query) {
        ...
        if (type == BigType.EDGE) {
            ...
            if (!query.conditions().isEmpty() &&
                ...
                // Does query IN edges
                if (cq.condition(BigKeys.DIRECTION) == Directions.IN) {
                    type = BigType.EDGE_IN;
                }
            }
        }
    ...
    public static abstract class ShardSpliter<Session extends BackendSession> {
        ...
        public List<io.bigconnect.biggraph.backend.store.Shard> getSplits(Session session, long splitSize) {
            ...
            List<io.bigconnect.biggraph.backend.store.Shard> splits = new ArrayList<>((int) count);
            String last = START;
            long offset = 0L;
            while (offset < maxKey) {
                offset += each;
                if (offset > maxKey) {
                    splits.add(new io.bigconnect.biggraph.backend.store.Shard(last, END, 0L));
                    break;
                }
                String current = this.position(offset);
                splits.add(new io.bigconnect.biggraph.backend.store.Shard(last, current, 0L));
                last = current;
            }
            return splits;
        }
        ...
        public static class Range {
            ...
            public List<io.bigconnect.biggraph.backend.store.Shard> splitEven(int count) {
                if (count <= 1) {
                    return ImmutableList.of(new io.bigconnect.biggraph.backend.store.Shard(startKey(this.startKey),
                ...
                List<io.bigconnect.biggraph.backend.store.Shard> shards = new ArrayList<>(count);
                while (Bytes.compare(offset, end) < 0 && !finished) {
                    ...
                    shards.add(new io.bigconnect.biggraph.backend.store.Shard(startKey(last), endKey(offset), 0));
                    last = offset;
                }
                return shards;
            }
