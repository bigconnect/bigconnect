...
import io.bigconnect.biggraph.backend.store.BackendSession;
...
public class InMemoryDBTables {
    ...
    public static class Edge extends InMemoryDBTable {
        ...
        public void insert(BackendSession session, TextBackendEntry entry) {
            ...
            if (!this.store().containsKey(id)) {
                BackendEntry vertex = new TextBackendEntry(BigType.VERTEX, id);
                vertex.merge(entry);
        ...
        protected Map<Id, BackendEntry> queryByIdPrefix(
            ...
            BackendEntry edges = new TextBackendEntry(BigType.VERTEX,
                                                      entry.id());
        ...
        protected Map<Id, BackendEntry> queryByFilter(
            ...
            E.checkState(relation.key().equals(BigKeys.LABEL),
                         "Not support querying edge by %s", conditions);
            ...
            for (BackendEntry value : entries.values()) {
                // TODO: Compatible with BackendEntry
                TextBackendEntry entry = (TextBackendEntry) value;
                String out = EdgeId.concat(BigType.EDGE_OUT.string(), label);
                String in = EdgeId.concat(BigType.EDGE_IN.string(), label);
    ...
    public static class SecondaryIndex extends InMemoryDBTable {
        ...
        public SecondaryIndex() {
            super(BigType.SECONDARY_INDEX);
        }

        protected SecondaryIndex(BigType type) {
            super(type);
        }
        ...
        public Iterator<BackendEntry> query(BackendSession session,
            ...
            for (Condition c : conditions) {
                ...
                if (r.key() == BigKeys.FIELD_VALUES) {
                    fieldValue = r.value().toString();
                } else if (r.key() == BigKeys.INDEX_LABEL_ID) {
            ...
            Id id = BigIndex.formatIndexId(query.resultType(),
        ...
        public void delete(BackendSession session, TextBackendEntry entry) {
            ...
            String indexLabel = entry.column(BigKeys.INDEX_LABEL_ID);
    ...
    public static class UniqueIndex extends SecondaryIndex {
        ...
        public UniqueIndex() {
            super(BigType.UNIQUE_INDEX);
        }
    }

    public static class RangeIndex extends InMemoryDBTable {

        protected RangeIndex(BigType type) {
        ...
        public Iterator<BackendEntry> query(BackendSession session,
            ...
            for (Condition.Relation r : ((ConditionQuery) query).relations()) {
                if (r.key().equals(BigKeys.INDEX_LABEL_ID)) {
                    indexLabelId = (Id) r.value();
                    continue;
                }
                relations.add(r);
        ...
        private Iterator<BackendEntry> betweenQuery(Id indexLabelId,
            ...
            Id min = BigIndex.formatIndexId(type, indexLabelId, keyMin);
            ...
            Id max = BigIndex.formatIndexId(type, indexLabelId, keyMax);
            ...
            Entry<Id, BackendEntry> entry = keyMinEq ?
                                                rs.ceilingEntry(min) :
                                                rs.higherEntry(min);
            while (entry != null) {
        ...
        public void delete(BackendSession session, TextBackendEntry entry) {
            ...
            Id min = BigIndex.formatIndexId(entry.type(), indexLabelId, 0L);
            indexLabelId = IdGenerator.of(indexLabelId.asLong() + 1L);
            Id max = BigIndex.formatIndexId(entry.type(), indexLabelId, 0L);
        ...
        public static RangeIndex rangeInt() {
            return new RangeIndex(BigType.RANGE_INT_INDEX);
        }

        public static RangeIndex rangeFloat() {
            return new RangeIndex(BigType.RANGE_FLOAT_INDEX);
        }

        public static RangeIndex rangeLong() {
            return new RangeIndex(BigType.RANGE_LONG_INDEX);
        }

        public static RangeIndex rangeDouble() {
            return new RangeIndex(BigType.RANGE_DOUBLE_INDEX);
        }
    }
