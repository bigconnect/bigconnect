...
package io.bigconnect.biggraph.backend.store.raft;
...
import io.bigconnect.biggraph.backend.store.raft.rpc.ListPeersProcessor;
import io.bigconnect.biggraph.backend.store.raft.rpc.RaftRequests.StoreType;
import io.bigconnect.biggraph.backend.store.raft.rpc.RpcForwarder;
import io.bigconnect.biggraph.backend.store.raft.rpc.SetLeaderProcessor;
import io.bigconnect.biggraph.backend.store.raft.rpc.StoreCommandProcessor;
...
public final class RaftSharedContext {
    ...
    public RaftSharedContext(BigGraphParams params) {
        this.params = params;
        BigConfig config = params.configuration();
    ...
    public NodeOptions nodeOptions() throws IOException {
        ...
        if (!groupPeers.parse(groupPeersStr)) {
            throw new BigGraphException("Failed to parse group peers %s",
                                    groupPeersStr);
        }
    ...
    public void clearCache() {
        ...
        this.notifyCache(Cache.ACTION_CLEAR, BigType.VERTEX_LABEL, null);
        this.notifyCache(Cache.ACTION_CLEAR, BigType.VERTEX, null);
    }

    protected void notifyCache(String action, BigType type, List<Id> ids) {
        EventHub eventHub;
        if (type.isGraph()) {
    ...
    public PeerId endpoint() {
        ...
        if (!endpoint.parse(endpointStr)) {
            throw new BigGraphException("Failed to parse endpoint %s", endpointStr);
        }
        return endpoint;
    }
