...
package io.bigconnect.biggraph.backend.store.ram;
...
import io.bigconnect.biggraph.util.Consumers;
...
public final class RamTable {
    ...
    public RamTable(BigGraph graph) {
        this(graph, VERTICES_CAPACITY, EDGES_CAPACITY);
    }

    public RamTable(BigGraph graph, long maxVertices, int maxEdges) {
        this.graph = graph;
    ...
    public void reload(boolean loadFromFile, String file) {
        if (this.loading) {
            throw new BigGraphException("There is one loading task, " +
        ...
        try {
            ...
            this.reset();
            throw new BigGraphException("Failed to load ramtable", e);
        } finally {
            this.loading = false;
        }
    }
    ...
    private void loadFromDB() throws Exception {
        Query query = new Query(BigType.VERTEX);
        ...
        while (vertices.hasNext()) {
            ...
            if (vertex.compareTo(lastId) < 0) {
                throw new BigGraphException("The ramtable feature is not " +
            ...
            if (adjEdges.hasNext()) {
                BigEdge edge = (BigEdge) adjEdges.next();
    ...
    public void addEdge(boolean newVertex, BigEdge edge) {
        if (edge.schemaLabel().existSortKeys()) {
            throw new BigGraphException("Only edge label without sortkey is " +
    ...
    public boolean matched(Query query) {
        ...
        Id owner = cq.condition(BigKeys.OWNER_VERTEX);
    ...
    @Watched
    public Iterator<BigEdge> query(Query query) {
        ...
        Id owner = cq.condition(BigKeys.OWNER_VERTEX);
        assert owner != null;
        Directions dir = cq.condition(BigKeys.DIRECTION);
        if (dir == null) {
            dir = Directions.BOTH;
        }
        Id label = cq.condition(BigKeys.LABEL);
    ...
    @Watched
    public Iterator<BigEdge> query(long owner, Directions dir, int label) {
        if (this.loading) {
    ...
    private void vertexAdjPosition(long vertex, int position) {
        if (vertex < this.verticesCapacityHalf) {
            ...
            throw new BigGraphException("Out of vertices capaticy %s",
    ...
    private int vertexAdjPosition(long vertex) {
        if (vertex < this.verticesCapacityHalf) {
            ...
            throw new BigGraphException("Out of vertices capaticy %s: %s",
    ...
    private static void ensureNumberId(Id id) {
        if (!id.number()) {
            throw new BigGraphException("Only number id is supported by " +
    ...
    private class EdgeRangeIterator implements Iterator<BigEdge> {

        private final int end;
        private final Directions dir;
        private final int label;
        private final BigVertex owner;
        private int current;
        private BigEdge currentEdge;
        ...
        public EdgeRangeIterator(int start, int end,
            ...
            this.dir = dir;
            this.label = label;
            this.owner = new BigVertex(RamTable.this.graph,
        ...
        public BigEdge next() {
            ...
            BigEdge edge = this.currentEdge;
            this.currentEdge = null;
            return edge;
        }

        private BigEdge fetch() {
            ...
            BigGraph graph = RamTable.this.graph;
            ...
            BigEdge edge = BigEdge.constructEdge(this.owner, direction,
    ...
    private class LoadTraverser implements AutoCloseable {
        ...
        private void addVertex(Id vertex) {
            ...
            if (!vertex.number()) {
                throw new BigGraphException("Only number id is supported " +
        ...
        private void addEdgesByBatch() {
            ...
            for (Id vertex : this.vertices) {
                ...
                for (int i = 0; i < adjEdges.size(); i++) {
                    BigEdge edge = (BigEdge) adjEdges.get(i);
