...
import io.bigconnect.biggraph.backend.page.*;
import io.bigconnect.biggraph.backend.page.IdHolder.BatchIdHolder;
import io.bigconnect.biggraph.backend.page.IdHolder.FixedIdHolder;
import io.bigconnect.biggraph.backend.page.IdHolder.PagingIdHolder;
...
public class GraphIndexTransaction extends AbstractTransaction {
    ...
    public GraphIndexTransaction(BigGraphParams graph, BackendStore store) {
        super(graph, store);
    ...
    protected Id asyncRemoveIndexLeft(ConditionQuery query,
        ...
        BigTask<?> task = EphemeralJobBuilder.of(this.graph())
    ...
    public void updateLabelIndex(BigElement element, boolean removed) {
        if (element instanceof BigVertex && ((BigVertex) element).olap()) {
            return;
        }
        ...
        BigIndex index = new BigIndex(this.graph(),
    ...
    @Watched(prefix = "index")
    public void updateVertexIndex(BigVertex vertex, boolean removed) {
        if (vertex.olap()) {
    ...
    @Watched(prefix = "index")
    public void updateEdgeIndex(BigEdge edge, boolean removed) {
    ...
    private void updateVertexOlapIndex(BigVertex vertex, boolean removed) {
     ...
     */
    protected void updateIndex(Id ilId, BigElement element, boolean removed) {
        ...
        for (Id fieldId : indexLabel.indexFields()) {
            BigProperty<Object> property = element.getProperty(fieldId);
            if (property == null) {
    ...
    private boolean existUniqueValueInStore(IndexLabel indexLabel,
                                            Object value) {
        ConditionQuery query = new ConditionQuery(BigType.UNIQUE_INDEX);
        query.eq(BigKeys.INDEX_LABEL_ID, indexLabel.id());
        query.eq(BigKeys.FIELD_VALUES, value);
        boolean exist;
        Iterator<BackendEntry> iterator = this.query(query).iterator();
        try {
            exist = iterator.hasNext();
            if (exist) {
                BigIndex index = this.serializer.readIndex(graph(), query,
                                                            iterator.next());
    ...
    public IdHolderList queryIndex(ConditionQuery query) {
        ...
        if (this.hasUpdate()) {
            throw new BigGraphException("Can't do index query when " +
        ...
        if (conds.size() > 1 ||
            (conds.size() == 1 && !query.containsCondition(BigKeys.LABEL))) {
            throw new BigGraphException("Can't do index query with %s and %s",
        ...
        if (query.allSysprop() && conds.size() == 1 &&
            query.containsCondition(BigKeys.LABEL)) {
            // Query only by label
    ...
    private IdHolderList queryByLabel(ConditionQuery query) {
        BigType queryType = query.resultType();
        ...
        if (queryType.isVertex()) {
            indexType = BigType.VERTEX_LABEL_INDEX;
            schemaLabel = this.graph().vertexLabel(label);
        } else if (queryType.isEdge()) {
            indexType = BigType.EDGE_LABEL_INDEX;
            schemaLabel = this.graph().edgeLabel(label);
        } else {
            throw new BigGraphException("Can't query %s by label", queryType);
        }
        ...
        indexQuery.eq(BigKeys.FIELD_VALUES, label);
        /*
    ...
    private void recordIndexValue(ConditionQuery query, BigIndex index) {
        ...
        query.registerResultsFilter(elem -> {
            for (Condition cond : originQuery.conditions()) {
                ...
                if (key instanceof Id && indexFields.contains(key)) {
                    ...
                    Id field = (Id) key;
                    assert elem != null;
                    BigProperty<?> property = elem.getProperty(field);
    ...
    private void removeExpiredIndexIfNeeded(BigIndex index,
                                            boolean showExpired) {
        if (this.store().features().supportsTtl() || showExpired) {
            return;
        }
        for (BigIndex.IdWithExpiredTime id : index.expiredElementIds()) {
            BigIndex removeIndex = index.clone();
    ...
    private static ConditionQuery constructQuery(ConditionQuery query,
        ...
        switch (indexType) {
            case SEARCH:
                ...
                indexQuery.eq(BigKeys.INDEX_LABEL_ID, indexLabel.id());
                indexQuery.eq(BigKeys.FIELD_VALUES, fieldValue);
                break;
            case SECONDARY:
                ...
                indexQuery.eq(BigKeys.INDEX_LABEL_ID, indexLabel.id());
                indexQuery.eq(BigKeys.FIELD_VALUES, joinedValues);
                break;
            case RANGE_INT:
            case RANGE_FLOAT:
            case RANGE_LONG:
            case RANGE_DOUBLE:
                ...
                break;
            case SHARD:
                BigType type = indexLabel.indexType().type();
    ...
    protected static List<Condition> constructShardConditions(
         ...
         * layer can do filter.
         */
        if (key == BigKeys.FIELD_VALUES &&
            processedCondCount < query.userpropKeys().size()) {
            throw new BigGraphException("Invalid shard index query: %s", query);
        }
    ...
    private static Relation shardFieldValuesCondition(BigKeys key,
    ...
    private static boolean validQueryConditionValues(BigGraph graph,
                                                     ConditionQuery query) {
    ...
    private static NoIndexException noIndexException(BigGraph graph,
                                                     ConditionQuery query,
                                                     Id label) {
    ...
    private static boolean hasNullableProp(BigElement element, Id key) {
        return element.schemaLabel().nullableKeys().contains(key);
    }

    private static Set<IndexLabel> relatedIndexLabels(BigElement element) {
        Set<IndexLabel> indexLabels = InsertionOrderUtil.newSet();
    ...
    private static class IndexQueries
        ...
        public Entry<IndexLabel, ConditionQuery> one() {
        ...
        private static class JointQuery extends Query {
            ...
            public JointQuery(BigType type, Collection<Query> queries) {
    ...
    public static class RemoveLeftIndexJob extends EphemeralJob<Object> {
        ...
        private final BigElement element;
        private GraphIndexTransaction tx;
        private Set<ConditionQuery.LeftIndex> leftIndexes;

        private RemoveLeftIndexJob(ConditionQuery query, BigElement element) {
        ...
        protected long removeIndexLeft(ConditionQuery query,
                                       BigElement element) {
            if (element.type() != BigType.VERTEX &&
                element.type() != BigType.EDGE_OUT &&
                element.type() != BigType.EDGE_IN) {
                throw new BigGraphException("Only accept element of type VERTEX " +
        ...
        private long processSecondaryOrSearchIndexLeft(ConditionQuery query,
            ...
            BigElement deletion = this.constructErrorElem(query, element,
                                                           incorrectPKs);
            if (deletion == null) {
                return 0;
            }
        ...
        private BigElement constructErrorElem(
                            ConditionQuery query, BigElement element,
                            Map<PropertyKey, Object> incorrectPKs) {
            BigElement errorElem = element.copyAsFresh();
            ...
            for (Id key : propKeys) {
                ...
                BigProperty<?> prop = element.getProperty(key);
        ...
        private boolean deletedByError(ConditionQuery query,
                                       BigElement element) {
            BigElement elem = this.newestElement(element);
            if (elem == null) {
                return false;
            }
        ...
        private boolean deletedByError(BigElement element,
        ...
        private BigElement newestElement(BigElement element) {
            boolean isVertex = element instanceof BigVertex;
            if (isVertex) {
                Iterator<Vertex> iter = this.graph().vertices(element.id());
                return (BigVertex) QueryResults.one(iter);
            } else {
                assert element instanceof BigEdge;
                Iterator<Edge> iter = this.graph().edges(element.id());
                return (BigEdge) QueryResults.one(iter);
            }
        }
    }
}
