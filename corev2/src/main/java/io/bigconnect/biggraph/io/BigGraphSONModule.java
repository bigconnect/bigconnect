...
import io.bigconnect.biggraph.backend.id.IdGenerator.LongId;
import io.bigconnect.biggraph.backend.id.IdGenerator.StringId;
import io.bigconnect.biggraph.backend.id.IdGenerator.UuidId;
import io.bigconnect.biggraph.backend.store.Shard;
...
import io.bigconnect.biggraph.util.Blob;

@SuppressWarnings("serial")
public class BigGraphSONModule extends TinkerPopJacksonModule {

    private static final long serialVersionUID = 6480426922914059122L;

    private static final String TYPE_NAMESPACE = "biggraph";
    ...
    static {
        ...
        TYPE_DEFINITIONS.put(BigVertex.class, "HugeVertex");
    ...
    public static void register(BigGraphIoRegistry io) {
        io.register(GraphSONIo.class, null, new BigGraphSONModule());
    }

    private BigGraphSONModule() {
    ...
    public static void registerGraphSerializers(SimpleModule module) {
         ...
         */
        module.addSerializer(BigVertex.class, new BigVertexSerializer());
        module.addSerializer(BigEdge.class, new BigEdgeSerializer());
    ...
    protected static abstract class BigElementSerializer<T extends BigElement>
                              extends StdSerializer<T> {

        public BigElementSerializer(Class<T> clazz) {
            super(clazz);
        }
        ...
        public void writePropertiesField(Map<Id, BigProperty<?>> properties,
                                         JsonGenerator generator,
                                         SerializerProvider provider)
                                         throws IOException {
            ...
            for (BigProperty<?> property : properties.values()) {
    ...
    private static class BigVertexSerializer
                   extends BigElementSerializer<BigVertex> {

        public BigVertexSerializer() {
            super(BigVertex.class);
        }

        @Override
        public void serialize(BigVertex vertex, JsonGenerator generator,
                              SerializerProvider provider)
                              throws IOException {
            vertex.forceLoad();
            generator.writeStartObject();
        ...
        @Override
        public void serializeWithType(BigVertex value, JsonGenerator generator,
                                      SerializerProvider provider,
                                      TypeSerializer typeSer)
                                      throws IOException {
            WritableTypeId typeId = typeSer.typeId(value, JsonToken.VALUE_STRING);
    ...
    private static class BigEdgeSerializer
                   extends BigElementSerializer<BigEdge> {

        public BigEdgeSerializer() {
            super(BigEdge.class);
        }

        @Override
        public void serialize(BigEdge edge, JsonGenerator generator,
                              SerializerProvider provider)
                              throws IOException {
            edge.forceLoad();
            generator.writeStartObject();
            ...
            BigVertex outVertex = (BigVertex) edge.outVertex();
            BigVertex inVertex = (BigVertex) edge.inVertex();
        ...
        @Override
        public void serializeWithType(BigEdge value, JsonGenerator generator,
                                      SerializerProvider provider,
                                      TypeSerializer typeSer)
                                      throws IOException {
            WritableTypeId typeId = typeSer.typeId(value, JsonToken.VALUE_STRING);
