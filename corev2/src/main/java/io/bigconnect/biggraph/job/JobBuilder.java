...
import io.bigconnect.biggraph.task.TaskCallable;
...
public class JobBuilder<V> {
    ...
    public static <T> JobBuilder<T> of(final BigGraph graph) {
        return new JobBuilder<>(graph);
    }

    public JobBuilder(final BigGraph graph) {
        this.graph = graph;
    }
    ...
    public BigTask<V> schedule() {
        ...
        BigTask<V> task = new BigTask<>(this.genTaskId(), null, job);
    ...
    private Id genTaskId() {
        return this.graph.getNextId(BigType.TASK);
    }
}
