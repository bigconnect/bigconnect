...
import io.bigconnect.biggraph.schema.Userdata;
...
import io.bigconnect.biggraph.type.define.Frequency;
...
public class EdgeLabelBuilder extends AbstractBuilder
    ...
    public EdgeLabelBuilder(SchemaTransaction transaction,
                            BigGraph graph, EdgeLabel copy) {
        super(transaction, graph);
        E.checkNotNull(copy, "copy");
        BigGraph origin = copy.graph();
        this.id = null;
        this.name = copy.name();
    ...
    public EdgeLabel build() {
        Id id = this.validOrGenerateId(BigType.EDGE_LABEL,
                                       this.id, this.name);
    ...
    public EdgeLabel create() {
        BigType type = BigType.EDGE_LABEL;
    ...
    private static Set<String> mapPkId2Name(BigGraph graph, Set<Id> ids) {
        return new HashSet<>(graph.mapPkId2Name(ids));
    }

    private static List<String> mapPkId2Name(BigGraph graph, List<Id> ids) {
