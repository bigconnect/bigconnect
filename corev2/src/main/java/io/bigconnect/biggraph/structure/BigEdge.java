...
public class BigEdge extends BigElement implements Edge, Cloneable {

    private Id id;
    private EdgeLabel label;
    private String name;

    private BigVertex sourceVertex;
    private BigVertex targetVertex;
    private boolean isOutEdge;

    public BigEdge(BigVertex owner, Id id, EdgeLabel label,
                   BigVertex other) {
    ...
    public BigEdge(final BigGraph graph, Id id, EdgeLabel label) {
        super(graph);
    ...
    public BigType type() {
        ...
        return this.isOutEdge ? BigType.EDGE_OUT : BigType.EDGE_IN;
    }
    ...
    public List<Object> sortValues() {
        ...
        for (Id sk : sortKeys) {
            BigProperty<?> property = this.getProperty(sk);
    ...
    @Watched(prefix = "edge")
    @Override
    protected <V> BigEdgeProperty<V> newProperty(PropertyKey pkey, V val) {
        return new BigEdgeProperty<>(this, pkey, val);
    }
    ...
    protected <V> void onUpdateProperty(Cardinality cardinality,
                                        BigProperty<V> prop) {
        if (prop != null) {
            assert prop instanceof BigEdgeProperty;
            BigEdgeProperty<V> edgeProp = (BigEdgeProperty<V>) prop;
            GraphTransaction tx = this.tx();
            if (tx != null) {
                assert this.fresh();
    ...
    protected boolean ensureFilledProperties(boolean throwIfNotExist) {
        ...
        this.copyProperties((BigEdge) edge);
        this.updateToDefaultValueIfNone();
        return true;
    }
    ...
    public void vertices(BigVertex owner, BigVertex other) {
    ...
    public void vertices(boolean outEdge, BigVertex owner, BigVertex other) {
    ...
    @Watched
    public BigEdge switchOwner() {
        BigEdge edge = this.clone();
        edge.isOutEdge = !edge.isOutEdge;
        edge.id = ((EdgeId) edge.id).switchDirection();
        return edge;
    }

    public BigEdge switchToOutDirection() {
    ...
    public BigVertex ownerVertex() {
        return this.isOutEdge ? this.sourceVertex() : this.targetVertex();
    }

    public BigVertex sourceVertex() {
        this.checkAdjacentVertexExist(this.sourceVertex);
        return this.sourceVertex;
    }

    public void sourceVertex(BigVertex sourceVertex) {
        this.sourceVertex = sourceVertex;
    }

    public BigVertex targetVertex() {
        this.checkAdjacentVertexExist(this.targetVertex);
        return this.targetVertex;
    }

    public void targetVertex(BigVertex targetVertex) {
        this.targetVertex = targetVertex;
    }

    private void checkAdjacentVertexExist(BigVertex vertex) {
        if (vertex.schemaLabel().undefined() &&
            this.graph().checkAdjacentVertexExist()) {
            throw new BigGraphException("Vertex '%s' does not exist", vertex.id());
        }
    }
    ...
    public boolean belongToVertex(BigVertex vertex) {
        return vertex != null && (vertex.equals(this.sourceVertex) ||
                                  vertex.equals(this.targetVertex));
    }

    public BigVertex otherVertex(BigVertex vertex) {
    ...
    @Override
    protected BigEdge clone() {
        try {
            return (BigEdge) super.clone();
        } catch (CloneNotSupportedException e) {
            throw new BigGraphException("Failed to clone HugeEdge", e);
        }
    }
    ...
    public static final EdgeId getIdValue(Object idValue,
        ...
        Id id = BigElement.getIdValue(idValue);
    ...
    @Watched
    public static BigEdge constructEdge(BigVertex ownerVertex,
                                        boolean isOutEdge,
                                        EdgeLabel edgeLabel,
                                        String sortValues,
                                        Id otherVertexId) {
        BigGraph graph = ownerVertex.graph();
        ...
        BigVertex otherVertex = new BigVertex(graph, otherVertexId,
                                                otherVertexLabel);

        ownerVertex.propNotLoaded();
        otherVertex.propNotLoaded();

        BigEdge edge = new BigEdge(graph, null, edgeLabel);
