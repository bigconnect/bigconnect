...
public class BigVertex extends BigElement implements Vertex, Cloneable {

    private static final List<BigEdge> EMPTY_LIST = ImmutableList.of();

    private Id id;
    private VertexLabel label;
    protected Collection<BigEdge> edges;

    public BigVertex(final BigGraph graph, Id id, VertexLabel label) {
        super(graph);
    ...
    public List<Object> primaryValues() {
        ...
        for (Id pk : primaryKeys) {
            BigProperty<?> property = this.getProperty(pk);
    ...
    public Collection<BigEdge> getEdges() {
    ...
    public void addEdge(BigEdge edge) {
    ...
    @Watched(prefix = "vertex")
    @Override
    public BigEdge addEdge(String label, Vertex vertex, Object... keyValues) {
        BigVertex targetVertex = (BigVertex) vertex;

        BigEdge edge = this.constructEdge(label, targetVertex, keyValues);
        ...
        if (this.fresh()) {
            ...
            return (BigEdge) this.graph().addEdge(edge);
        }
    }

    public BigEdge constructEdge(String label, BigVertex vertex,
                                 Object... keyValues) {
        ElementKeys elemKeys = BigElement.classifyKeys(keyValues);
        // Check id (must be null)
        if (elemKeys.id() != null) {
            throw Edge.Exceptions.userSuppliedIdsNotSupported();
        }
        Id id = BigElement.getIdValue(elemKeys.id());

        // Check target vertex
        E.checkArgumentNotNull(vertex, "Target vertex can't be null");
        E.checkArgument(vertex instanceof BigVertex,
        ...
        BigEdge edge = new BigEdge(this, id, edgeLabel, vertex);
     ...
     */
    @Watched
    public void addOutEdge(BigEdge edge) {
     ...
     */
    @Watched
    public void addInEdge(BigEdge edge) {
        if (edge.ownerVertex() == null) {
    ...
    @Watched(prefix = "vertex")
    @Override
    protected <V> BigVertexProperty<V> newProperty(PropertyKey pkey, V val) {
        return new BigVertexProperty<>(this, pkey, val);
    }
    ...
    protected <V> void onUpdateProperty(Cardinality cardinality,
                                        BigProperty<V> prop) {
        if (prop != null) {
            assert prop instanceof BigVertexProperty;
            /*
            ...
            if (tx != null) {
                assert this.fresh();
                tx.addVertexProperty((BigVertexProperty<V>) prop);
            } else {
                assert !this.fresh();
                this.graph().addVertexProperty((BigVertexProperty<V>) prop);
            }
        }
    }
    ...
    public <V> Iterator<VertexProperty<V>> properties(String... keys) {
        ...
        if (keys.length == 0) {
            for (BigProperty<?> prop : this.getProperties().values()) {
            ...
            for (String key : keys) {
                ...
                BigProperty<?> prop = this.getProperty(pkeyId);
                if (prop == null) {
                    // Not found
                    continue;
                }
    ...
    @Override
    public Object sysprop(BigKeys key) {
        switch (key) {
            case ID:
                return this.id();
            case LABEL:
    ...
    @Override
    protected BigVertex clone() {
        try {
            return (BigVertex) super.clone();
        } catch (CloneNotSupportedException e) {
            throw new BigGraphException("Failed to clone HugeVertex", e);
        }
    }
    ...
    public static final Id getIdValue(Object idValue) {
        return BigElement.getIdValue(idValue);
    }

    public static BigVertex undefined(BigGraph graph, Id id) {
        VertexLabel label = VertexLabel.undefined(graph);
        return new BigVertex(graph, id, label);
    }

    public static BigVertex create(final GraphTransaction tx,
                                   Id id, VertexLabel label) {
        return new BigVertex4Insert(tx, id, label);
    }
    ...
    private static final class BigVertex4Insert extends BigVertex {

        private GraphTransaction tx;

        public BigVertex4Insert(final GraphTransaction tx,
                                Id id, VertexLabel label) {
