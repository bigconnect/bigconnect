...
import io.bigconnect.biggraph.job.ComputerJob;
...
public class BigTask<V> extends FutureTask<V> {

    private static final Logger LOG = Log.logger(BigTask.class);
    ...
    public BigTask(Id id, Id parent, String callable, String input) {
        this(id, parent, TaskCallable.fromClass(callable));
        this.input(input);
    }

    public BigTask(Id id, Id parent, TaskCallable<V> callable) {
        super(callable);
    ...
    public boolean fail(Throwable e) {
        ...
        if (!(this.cancelled() && BigGraphException.isInterrupted(e))) {
    ...
    protected boolean checkDependenciesSuccess() {
        ...
        for (Id dependency : this.dependencies) {
            BigTask<?> task = scheduler.task(dependency);
            if (!task.completed()) {
    ...
    public static <V> BigTask<V> fromVertex(Vertex vertex) {
        ...
        BigTask<V> task = new BigTask<>((Id) vertex.id(), null, callable);
    ...
    private void checkPropertySize(int propertyLength, String propertyName) {
        ...
        BigGraph graph = this.scheduler().graph();
    ...
    public void syncWait() {
        ...
        try {
            ...
            throw new BigGraphException("Failed to wait for task '%s' completed",
                                    e, this.id);
        }
        assert task != null;
        /*
        ...
        if (debugTest && !task.success()) {
            throw new BigGraphException("Task '%s' is failed with error: %s",
                                    task.id(), task.result());
        }
    }
