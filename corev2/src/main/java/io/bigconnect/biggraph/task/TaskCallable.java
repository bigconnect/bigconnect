...
public abstract class TaskCallable<V> implements Callable<V> {
    ...
    private BigTask<V> task = null;
    private BigGraph graph = null;
    ...
    protected void graph(BigGraph graph) {
        this.graph = graph;
    }
    ...
    protected void task(BigTask<V> task) {
        this.task = task;
    }

    public BigTask<V> task() {
    ...
    public static <V> TaskCallable<V> fromClass(String className) {
        try {
            ...
            throw new BigGraphException("Failed to load task: %s", e, className);
        }
    }
    ...
    public static abstract class SysTaskCallable<V> extends TaskCallable<V> {
        ...
        private BigGraphParams params = null;

        protected void params(BigGraphParams params) {
            this.params = params;
        }
