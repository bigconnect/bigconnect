...
import io.bigconnect.biggraph.backend.query.Aggregate;
...
import io.bigconnect.biggraph.iterator.LimitIterator;
...
import io.bigconnect.biggraph.util.collection.MultivaluedHashMap;
...
public class BigTraverser {
    ...
    public BigTraverser(BigGraph graph) {
        this.graph = graph;
    ...
    protected Set<Id> adjacentVertices(Id source, EdgeStep step) {
        ...
        while (edges.hasNext()) {
            neighbors.add(((BigEdge) edges.next()).id().otherVertexId());
        }
        return neighbors;
    }
    ...
    private Iterator<Edge> edgesOfVertex(Id source, EdgeStep edgeStep,
        ...
        if (filter != null) {
            ...
            edges = new FilterIterator<>(edges, (e) -> {
                return finalFilter.test((BigEdge) e);
            });
        }
        return edgeStep.skipSuperNodeIfNeeded(edges);
    }
    ...
    private void fillFilterBySortKeys(Query query, Id[] edgeLabels,
        ...
        if (!GraphTransaction.matchFullEdgeSortKeys(condQuery, this.graph())) {
            Id label = condQuery.condition(BigKeys.LABEL);
    ...
    protected Object getVertexLabelId(Object label) {
        ...
        return SchemaLabel.getLabelId(this.graph, BigType.VERTEX, label);
    }
    ...
    protected Id getEdgeLabelId(Object label) {
        ...
        return SchemaLabel.getLabelId(this.graph, BigType.EDGE, label);
    }
    ...
    public static void checkCapacity(long capacity, long access,
        ...
        if (capacity != NO_LIMIT && access > capacity) {
            throw new BigGraphException("Exceed capacity '%s' while finding %s",
                                    capacity, traverse);
        }
    }
    ...
    public static class Node {
        ...
        public List<Id> joinPath(Node back) {
            return BigTraverser.joinPath(this, back, false);
        }
