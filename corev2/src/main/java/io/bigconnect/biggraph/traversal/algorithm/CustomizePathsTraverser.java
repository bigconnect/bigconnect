...
import io.bigconnect.biggraph.traversal.algorithm.steps.WeightedEdgeStep;
...
public class CustomizePathsTraverser extends BigTraverser {

    public CustomizePathsTraverser(BigGraph graph) {
        super(graph);
    }
    ...
    public static List<Path> topNPath(List<Path> paths,
        ...
        if (limit == BigTraverser.NO_LIMIT || paths.size() <= limit) {
            return paths;
        }
