...
public class FusiformSimilarityTraverser extends BigTraverser {

    private long accessed = 0L;

    public FusiformSimilarityTraverser(BigGraph graph) {
        super(graph);
    }
    ...
    private Set<Similar> fusiformSimilarityForVertex(
            BigVertex vertex, Directions direction,
        ...
        while (edges.hasNext()) {
            ...
            while (backEdges.hasNext()) {
                Id node = ((BigEdge) backEdges.next()).id().otherVertexId();
    ...
    private boolean matchMinNeighborCount(BigVertex vertex,
