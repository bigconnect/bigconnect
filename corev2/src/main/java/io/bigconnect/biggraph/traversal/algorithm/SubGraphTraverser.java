...
public class SubGraphTraverser extends BigTraverser {

    public SubGraphTraverser(BigGraph graph) {
        super(graph);
    }
    ...
    private static boolean hasMultiEdges(List<Edge> edges, Id target) {
        ...
        for (Edge edge : edges) {
            if (((BigEdge) edge).id().otherVertexId().equals(target)) {
                if (((BigEdge) edge).direction() == Directions.OUT) {
                    hasOutEdge = true;
                } else {
                    hasInEdge = true;
                }
