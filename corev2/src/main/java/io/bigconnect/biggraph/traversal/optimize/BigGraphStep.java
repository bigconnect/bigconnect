...
public final class BigGraphStep<S, E extends Element>
             extends GraphStep<S, E> implements QueryHolder {

    private static final long serialVersionUID = -679873894532085972L;

    private static final Logger LOG = Log.logger(BigGraphStep.class);

    private final List<HasContainer> hasContainers = new ArrayList<>();

    // Store limit/order-by
    private final Query queryInfo = new Query(BigType.UNKNOWN);

    private Iterator<E> lastTimeResults = QueryResults.emptyIterator();

    public BigGraphStep(final GraphStep<S, E> originGraphStep) {
    ...
    private long verticesCount() {
        if (!this.hasIds()) {
            ...
            Query query = this.makeQuery(graph, BigType.VERTEX);
    ...
    private long edgesCount() {
        if (!this.hasIds()) {
            BigGraph graph = TraversalUtil.getGraph(this);
            Query query = this.makeQuery(graph, BigType.EDGE);
            return graph.queryNumber(query).longValue();
        }
    ...
    private Query makeQuery(BigGraph graph, BigType type) {
        Query query = null;
