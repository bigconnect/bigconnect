...
public final class TraversalUtil {
    ...
    public static BigGraph getGraph(Step<?, ?> step) {
        return (BigGraph) step.getTraversal().getGraph().get();
    }

    public static void extractHasContainer(BigGraphStep<?, ?> newStep,
    ...
    public static void extractHasContainer(BigVertexStep<?> newStep,
                                           Traversal.Admin<?, ?> traversal) {
        Step<?, ?> step = newStep;
        do {
            if (step instanceof HasStep) {
    ...
    public static ConditionQuery fillConditionQuery(
                                 ...
                                 BigGraph graph) {
        BigType resultType = query.resultType();
    ...
    public static Condition convAnd(BigGraph graph,
                                    BigType type,
                                    HasContainer has) {
        P<?> p = has.getPredicate();
    ...
    public static Condition convOr(BigGraph graph,
                                   BigType type,
                                   HasContainer has) {
        P<?> p = has.getPredicate();
    ...
    private static Relation convCompare2Relation(BigGraph graph,
                                                 BigType type,
                                                 HasContainer has) {
        assert type.isGraph();
    ...
    private static Relation convCompare2SyspropRelation(BigGraph graph,
                                                        BigType type,
                                                        HasContainer has) {
        BiPredicate<?, ?> bp = has.getPredicate().getBiPredicate();
        assert bp instanceof Compare;

        BigKeys key = token2HugeKey(has.getKey());
    ...
    private static Relation convCompare2UserpropRelation(BigGraph graph,
                                                         BigType type,
                                                         HasContainer has) {
        BiPredicate<?, ?> bp = has.getPredicate().getBiPredicate();
        assert bp instanceof Compare;
    ...
    private static Condition convRelationType2Relation(BigGraph graph,
                                                       BigType type,
                                                       HasContainer has) {
        assert type.isGraph();
        BiPredicate<?, ?> bp = has.getPredicate().getBiPredicate();
    ...
    public static Condition convIn2Relation(BigGraph graph,
                                            BigType type,
                                            HasContainer has) {
        BiPredicate<?, ?> bp = has.getPredicate().getBiPredicate();
        ...
        BigKeys hugeKey = token2HugeKey(originKey);
        List<?> valueList;
        if (hugeKey != null) {
    ...
    public static Condition convContains2Relation(BigGraph graph,
                                                  HasContainer has) {
    ...
    public static BigKeys string2HugeKey(String key) {
        BigKeys hugeKey = token2HugeKey(key);
        return hugeKey != null ? hugeKey : BigKeys.valueOf(key);
    }

    public static BigKeys token2HugeKey(String key) {
        if (key.equals(T.label.getAccessor())) {
            return BigKeys.LABEL;
        } else if (key.equals(T.id.getAccessor())) {
            return BigKeys.ID;
        } else if (keyForContainsKeyOrValue(key)) {
            return BigKeys.PROPERTIES;
        }
        return null;
    }
    ...
    public static void convAllHasSteps(Traversal.Admin<?, ?> traversal) {
        ...
        BigGraph graph = (BigGraph) traversal.getGraph().get();
        for (HasStep<?> step : steps) {
            TraversalUtil.convHasStep(graph, step);
        }
    }

    public static void convHasStep(BigGraph graph, HasStep<?> step) {
    ...
    private static void convPredicateValue(BigGraph graph,
                                           HasContainer has) {
    ...
    private static Object convSysValueIfNeeded(BigGraph graph,
                                               BigType type,
                                               BigKeys key,
                                               Object value) {
        if (key == BigKeys.LABEL && !(value instanceof Id)) {
            value = SchemaLabel.getLabelId(graph, type, value);
        } else if (key == BigKeys.ID && !(value instanceof Id)) {
            value = BigElement.getIdValue(type, value);
        }
        return value;
    }

    private static List<?> convSysListValueIfNeeded(BigGraph graph,
                                                    BigType type,
                                                    BigKeys key,
                                                    Collection<?> values) {
    ...
    public static boolean testProperty(Property<?> prop, Object expected) {
        ...
        updatePredicateValue(predicate, ((BigProperty<?>) prop).propertyKey());
        return predicate.test(actual);
    }

    public static Map<Id, Object> transProperties(BigGraph graph,
    ...
    public static P<Object> parsePredicate(String predicate) {
        ...
        if (!matcher.find()) {
            throw new BigGraphException("Invalid predicate: %s", predicate);
        }
    ...
    private static Number[] predicateNumbers(String value, int count) {
        ...
        if (values.size() != count) {
            throw new BigGraphException("Invalid numbers size %s, expect %s",
                                    values.size(), count);
        }
