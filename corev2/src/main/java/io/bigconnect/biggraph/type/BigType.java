...
package io.bigconnect.biggraph.type;
...
public enum BigType implements SerialEnum {
    ...
    private static final Map<String, BigType> ALL_NAME = new HashMap<>();

    static {
        SerialEnum.register(BigType.class);
        for (BigType type : values()) {
            ALL_NAME.put(type.name, type);
        }
    }

    BigType(int type, String name) {
        assert type < 256;
        this.type = (byte) type;
        this.name = name;
    }
    ...
    public boolean isSchema() {
        return this == BigType.VERTEX_LABEL ||
               this == BigType.EDGE_LABEL ||
               this == BigType.PROPERTY_KEY ||
               this == BigType.INDEX_LABEL;
    }
    ...
    public static BigType fromString(String type) {
        return ALL_NAME.get(type);
    }

    public static BigType fromCode(byte code) {
        return SerialEnum.fromCode(BigType.class, code);
    }
}
