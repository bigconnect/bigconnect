...
import com.mware.ge.cypher.internal.expressions.{Property, Variable}
import com.mware.ge.cypher.internal.util.DummyPosition
import com.mware.ge.cypher.internal.util.symbols.{CTAny, CTInteger, CTMap, CTNode, CTNumber, CTRelationship, CTString, TypeSpec}
import com.mware.ge.cypher.internal.util.test_helpers.CypherFunSuite
import com.mware.ge.cypher.internal.expressions.{PropertyKeyName, Variable}
...
class SemanticStateTest extends CypherFunSuite {
  ...
  test("should expect type for expression") {
    val expression = expressions.DummyExpression(CTInteger | CTString | CTMap)
    val state = SemanticState.clean.specifyType(expression, expression.possibleTypes).right.get
