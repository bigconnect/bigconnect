...
trait StartPoints extends Parser
  ...
  private def IdentifiedIndexLookup: Rule3[String, String, com.mware.ge.cypher.internal.expressions.Expression] = rule {
    ":" ~~ SymbolicNameString ~~ "(" ~~ SymbolicNameString ~~ operator("=") ~~ (StringLiteral | Parameter) ~~ ")"
  }

  private def IndexQuery: Rule2[String, com.mware.ge.cypher.internal.expressions.Expression] = rule {
    ":" ~~ SymbolicNameString ~~ "(" ~~ (StringLiteral | Parameter) ~~ ")"
  }

  private def LiteralIds: Rule1[Seq[com.mware.ge.cypher.internal.expressions.UnsignedIntegerLiteral]] = rule("an unsigned integer") {
