...
import com.mware.ge.cypher.internal.util.attribution.{Id, IdGen, SameId}
import com.mware.ge.cypher.internal.util.test_helpers.CypherFunSuite
import com.mware.ge.cypher.internal.util.{InputPosition, LabelId, NonEmptyList, PropertyKeyId}
...
class IndexSeekTest extends CypherFunSuite {
  ...
  val testCaseCreators: List[(GetValueFromIndexBehavior, Set[String], IndexOrder) => (String, LogicalPlan)] = List(
    (getValue, args, indexOrder) => "a:X(prop = 1)" -> NodeIndexSeek("a", label("X"), List(prop("prop", getValue)), exactInt(1), args, indexOrder),
    (getValue, args, indexOrder) => "b:X(prop = 1)" -> NodeIndexSeek("b", label("X"), List(prop("prop", getValue)), exactInt(1), args, indexOrder),
    (getValue, args, indexOrder) => "b:Y(prop = 1)" -> NodeIndexSeek("b", label("Y"), List(prop("prop", getValue)), exactInt(1), args, indexOrder),
    (getValue, args, indexOrder) => "b:Y(dogs = 1)" -> NodeIndexSeek("b", label("Y"), List(prop("dogs", getValue)), exactInt(1), args, indexOrder),
    (getValue, args, indexOrder) => "b:Y(dogs = 2)" -> NodeIndexSeek("b", label("Y"), List(prop("dogs", getValue)), exactInt(2), args, indexOrder),
    (getValue, args, indexOrder) => "b:Y(dogs = 2, cats = 4)" -> NodeIndexSeek("b", label("Y"), List(prop("dogs", getValue), prop("cats", getValue)), CompositeQueryExpression(Seq(exactInt(2), exactInt(4))), args, indexOrder),
    (getValue, args, indexOrder) => "b:Y(name = 'hi')" -> NodeIndexSeek("b", label("Y"), List(prop("name", getValue)), exactString("hi"), args, indexOrder),
    (getValue, args, indexOrder) => "b:Y(name < 'hi')" -> NodeIndexSeek("b", label("Y"), List(prop("name", getValue)), lt(string("hi")), args, indexOrder),
    (getValue, args, indexOrder) => "b:Y(name <= 'hi')" -> NodeIndexSeek("b", label("Y"), List(prop("name", getValue)), lte(string("hi")), args, indexOrder),
    (getValue, args, indexOrder) => "b:Y(name > 'hi')" -> NodeIndexSeek("b", label("Y"), List(prop("name", getValue)), gt(string("hi")), args, indexOrder),
    (getValue, args, indexOrder) => "b:Y(name >= 'hi')" -> NodeIndexSeek("b", label("Y"), List(prop("name", getValue)), gte(string("hi")), args, indexOrder),
    (getValue, args, indexOrder) => "b:Y(name STARTS WITH 'hi')" -> NodeIndexSeek("b", label("Y"), List(prop("name", getValue)), startsWith("hi"), args, indexOrder),
  ...
  private def label(str:String) = LabelToken(str, LabelId(""))

  private def prop(name: String, getValue: GetValueFromIndexBehavior) =
    IndexedProperty(PropertyKeyToken(name, PropertyKeyId(name)), getValue)
