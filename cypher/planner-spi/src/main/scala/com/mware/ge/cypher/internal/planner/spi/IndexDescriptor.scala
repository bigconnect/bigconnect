...
import com.mware.ge.cypher.internal.logical.plans.{DoNotGetValue, GetValueFromIndexBehavior}
...
object IndexDescriptor {
  ...
  def apply(label: String, property: String): IndexDescriptor = IndexDescriptor(LabelId(label), Seq(PropertyKeyId(property)))

  implicit def toKernelEncode(properties: Seq[PropertyKeyId]): Array[String] = properties.map(_.id).toArray
}
