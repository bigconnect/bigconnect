 ...
 */
package com.mware.ge.cypher.internal.compiler.common;

import com.mware.ge.cypher.Path;
import com.mware.ge.cypher.internal.util.IncomparableValuesException;
import com.mware.ge.cypher.internal.util.UnorderableValueException;
import com.mware.ge.cypher.values.utils.ValueUtils;
import com.mware.ge.helpers.MathUtil;
...
public class CypherOrderability {
    ...
    public enum SuperType {
        ...
        public static SuperType ofValue(Object value) {
            if (value instanceof String || value instanceof Character) {
                ...
                if ("-1".equals(((VirtualNodeValue)value).id())) {
                    return VOID;
                }
                return NODE;
            } else if (value instanceof VirtualRelationshipValue) {
                if ("-1".equals(((VirtualRelationshipValue)value).id())) {
                    return VOID;
                }
                return RELATIONSHIP;
            }
    ...
    private static Comparator<VirtualNodeValue> NODE_COMPARATOR = Comparator.comparing(VirtualNodeValue::id);

    private static Comparator<VirtualRelationshipValue> RELATIONSHIP_COMPARATOR =
            Comparator.comparing(VirtualRelationshipValue::id);

    // TODO test
    private static Comparator<Path> PATH_COMPARATOR = (lhs, rhs) ->
    {
        Iterator<VirtualValue> lhsIter = lhs.iterator();
        Iterator<VirtualValue> rhsIter = rhs.iterator();
