...
import com.mware.ge.cypher.internal.compiler.planner.logical.cardinality.{IndependenceCombiner, SelectivityCombiner}
...
import com.mware.ge.cypher.internal.util.{Cardinality, Multiplier, Selectivity}
...
class StatisticsBackedCardinalityModel(queryGraphCardinalityModel: QueryGraphCardinalityModel, simpleExpressionEvaluator: ExpressionEvaluator) extends CardinalityModel {
  ...
  def apply(query: PlannerQuery, input0: QueryGraphSolverInput, semanticTable: SemanticTable, planContext: PlanContext): Cardinality = {
    val output = query.fold(input0) {
      case (input, RegularPlannerQuery(graph, _, horizon, _)) =>
        val QueryGraphSolverInput(newLabels, graphCardinality, laziness) = calculateCardinalityForQueryGraph(graph, input, semanticTable, planContext)

        val horizonCardinality = calculateCardinalityForQueryHorizon(graphCardinality, horizon, semanticTable)(planContext)
        QueryGraphSolverInput(newLabels, horizonCardinality, laziness)
    }
    output.inboundCardinality
  }

  private def calculateCardinalityForQueryHorizon(in: Cardinality, horizon: QueryHorizon, semanticTable: SemanticTable)
                                                 (implicit planContext: PlanContext): Cardinality = horizon match {
  ...
  private def horizonCardinalityWithSelections(cardinalityBeforeSelection: Cardinality,
                                              ...
                                              (implicit planContext: PlanContext): Cardinality = {
  ...
  private def calculateCardinalityForQueryGraph(graph: QueryGraph, input: QueryGraphSolverInput,
    ...
    val newCardinality = queryGraphCardinalityModel(graph, input, semanticTable, planContext)
