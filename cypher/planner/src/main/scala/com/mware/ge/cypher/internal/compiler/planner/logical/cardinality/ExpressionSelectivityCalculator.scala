...
import com.mware.ge.cypher.internal.compiler.planner.logical.plans._
import com.mware.ge.cypher.internal.ir.Selections
import com.mware.ge.cypher.internal.planner.spi.GraphStatistics._
...
case class ExpressionSelectivityCalculator(stats: GraphStatistics, combiner: SelectivityCombiner) {
  ...
  def apply(exp: Expression)(implicit semanticTable: SemanticTable, selections: Selections, planContext: PlanContext = null): Selectivity = exp match {
    // WHERE a:Label
    case HasLabels(_, label :: Nil) =>
      calculateSelectivityForLabel(semanticTable.id(label), planContext)
  ...
  private def calculateSelectivityForLabel(label: Option[LabelId], planContext: PlanContext): Selectivity = {
    val labelCardinality =
    if(label.isEmpty){
      Cardinality.SINGLE
    } else {
      label.map(l => stats.nodesWithLabelCardinality(Some(l), planContext)).getOrElse(Cardinality.SINGLE)
    }
  ...
  private def calculateSelectivityForPropertyExistence(variable: String,
                                                      ...
                                                      (implicit semanticTable: SemanticTable, planContext: PlanContext): Selectivity = {
  ...
  private def indexPropertyExistsSelectivitiesFor(variable: String,
                                                  selections: Selections,
                                                  propertyKey: PropertyKeyName)
                                                 (implicit semanticTable: SemanticTable, planContext: PlanContext): Seq[Selectivity] = {
      ...
      labelName =>
        (semanticTable.id(labelName), semanticTable.id(propertyKey)) match {
          case (Some(labelId), Some(propertyKeyId)) =>
            ...
            stats.indexPropertyExistsSelectivity(descriptor, planContext)
      ...
      labelName =>
        (semanticTable.id(labelName), semanticTable.id(propertyKey)) match {
          case (Some(labelId), Some(propertyKeyId)) =>
            val descriptor = IndexDescriptor(labelId, Seq(propertyKeyId))
            for {
              propExists <-stats.indexPropertyExistsSelectivity(descriptor, planContext)
              propEqualsValue <- stats.uniqueValueSelectivity(descriptor, planContext)
      ...
      labelName =>
        (semanticTable.id(labelName), semanticTable.id(seekable.expr.property.propertyKey)) match {
          case (Some(labelId), Some(propertyKeyId)) =>
            ...
            for {
              propertyExistsSelectivity <- stats.indexPropertyExistsSelectivity(descriptor, planContext)
              propEqValueSelectivity <- stats.uniqueValueSelectivity(descriptor, planContext)
            } yield {
