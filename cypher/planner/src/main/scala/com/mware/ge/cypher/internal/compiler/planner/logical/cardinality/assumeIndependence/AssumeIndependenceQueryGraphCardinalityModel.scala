...
import com.mware.ge.cypher.internal.compiler.planner.logical.Metrics.{QueryGraphCardinalityModel, QueryGraphSolverInput}
import com.mware.ge.cypher.internal.compiler.planner.logical.cardinality.{ExpressionSelectivityCalculator, SelectivityCombiner}
import com.mware.ge.cypher.internal.ast.semantics.SemanticTable
import com.mware.ge.cypher.internal.ir.{QueryGraph, _}
import com.mware.ge.cypher.internal.util.{Cardinality, Selectivity}
...
case class AssumeIndependenceQueryGraphCardinalityModel(stats: GraphStatistics, combiner: SelectivityCombiner)
   ...
   */
  def apply(queryGraph: QueryGraph, input: QueryGraphSolverInput, semanticTable: SemanticTable, planContext: PlanContext): Cardinality = {
    val combinations: Seq[QueryGraph] = findQueryGraphCombinations(queryGraph, input, semanticTable, planContext)

    val cardinalities = combinations.map(cardinalityForQueryGraph(_, input)(semanticTable, planContext))
    cardinalities.max
  }

  private def findQueryGraphCombinations(queryGraph: QueryGraph, input: QueryGraphSolverInput, semanticTable: SemanticTable, planContext: PlanContext): Seq[QueryGraph] =
    if (queryGraph.optionalMatches.isEmpty)
      ...
      val optionalMatches =
        ...
        else queryGraph.optionalMatches.sortBy(-cardinalityForQueryGraph(_, input)(semanticTable, planContext).amount).take(MAX_OPTIONAL_MATCH)
  ...
  private def cardinalityForQueryGraph(qg: QueryGraph, input: QueryGraphSolverInput)
                                      (implicit semanticTable: SemanticTable, planContext: PlanContext): Cardinality = {
  ...
  private def calculateSelectivity(qg: QueryGraph, labels: Map[String, Set[LabelName]])
                                  (implicit semanticTable: SemanticTable, planContext: PlanContext): (Selectivity, Int) = {
