 ...
 */
package com.mware.ge.cypher.internal.compiler.planner.logical.cardinality.assumeIndependence

import com.mware.ge.cypher.internal.planner.spi.{GraphStatistics, PlanContext}
import com.mware.ge.cypher.internal.compiler.planner.logical.cardinality._
import com.mware.ge.cypher.internal.ast.semantics.SemanticTable
import com.mware.ge.cypher.internal.ir.{Selections, _}
import com.mware.ge.cypher.internal.util.{Cardinality, LabelId, RelTypeId, Selectivity}
import com.mware.ge.cypher.internal.expressions.{LabelName, RelTypeName, SemanticDirection}

trait Pattern2Selectivity {
  def apply(pattern: PatternRelationship, labels: Map[String, Set[LabelName]])(implicit semanticTable: SemanticTable, selections: Selections, planContext: PlanContext): Selectivity
}
...
case class PatternSelectivityCalculator(stats: GraphStatistics, combiner: SelectivityCombiner) extends Pattern2Selectivity {
  ...
  import com.mware.ge.cypher.internal.compiler.planner.logical.cardinality.assumeIndependence.PatternSelectivityCalculator._

  def apply(pattern: PatternRelationship, labels: Map[String, Set[LabelName]])
           (implicit semanticTable: SemanticTable, selections: Selections, planContext: PlanContext): Selectivity = {
    ...
    val lhsCardinality = nbrOfNodesInGraph * calculateLabelSelectivity(labelsOnLhs, nbrOfNodesInGraph, planContext)
    val rhsCardinality = nbrOfNodesInGraph * calculateLabelSelectivity(labelsOnRhs, nbrOfNodesInGraph, planContext)
    ...
    if (lhsCardinality == Cardinality.EMPTY || rhsCardinality == Cardinality.EMPTY)
      ...
      pattern.length match {
        case SimplePatternLength =>
          calculateSelectivityForSingleRelHop(types, labelsOnLhs, labelsOnRhs, pattern.dir, estimatedCardinalityBasedOnLabels, planContext)
              ...
              yield {
                for (i <- 1 to length)
                  yield {
                    ...
                    calculateSelectivityForSingleRelHop(types, labelsOnL, labelsOnR, pattern.dir, estimatedMaxCardForPatternBasedOnLabels, planContext)
                  }
              }
  ...
  private def calculateSelectivityForSingleRelHop(types: Seq[TokenSpec[RelTypeId]],
                                                  ...
                                                  estimatedMaxCardForPatternBasedOnLabels: Cardinality,
                                                  planContext: PlanContext): Selectivity = {

    def selectivityEstimateForPattern(fromLabel: Option[LabelId], relType: Option[RelTypeId], toLabel: Option[LabelId], planContext: PlanContext): Selectivity = {
      stats.cardinalityByLabelsAndRelationshipType(fromLabel, relType, toLabel, planContext) / estimatedMaxCardForPatternBasedOnLabels getOrElse Selectivity.ONE
    }
      ...
      for {
        ...
        (lhsLabel, typ, rhsLabel) match {
          ...
          case _ if dir == SemanticDirection.OUTGOING =>
            selectivityEstimateForPattern(lhsLabel.id, typ.id, rhsLabel.id, planContext)

          case _ if dir == SemanticDirection.INCOMING =>
            selectivityEstimateForPattern(rhsLabel.id, typ.id, lhsLabel.id, planContext)
  ...
  private def calculateLabelSelectivity(specs: Seq[TokenSpec[LabelId]], totalNbrOfNodes: Cardinality, planContext: PlanContext): Selectivity = {
    val selectivities = specs map {
      ...
      case SpecifiedAndKnown(spec: LabelId) =>  // Specified labels have ids
          stats.nodesWithLabelCardinality(Some(spec), planContext) / totalNbrOfNodes getOrElse Selectivity.ZERO
    }
