...
import com.mware.ge.cypher.internal.compiler.helpers.ListSupport
import com.mware.ge.cypher.internal.compiler.helpers.PredicateHelper.coercePredicates
...
import com.mware.ge.cypher.internal.compiler.planner.logical.Metrics.CardinalityModel
import com.mware.ge.cypher.internal.ir._
import com.mware.ge.cypher.internal.logical.plans.{DeleteExpression => DeleteExpressionPlan, Limit => LimitPlan, LoadCSV => LoadCSVPlan, Skip => SkipPlan, _}
...
import com.mware.ge.cypher.internal.logical.plans.{DeleteExpression => DeleteExpressionPlan, Limit => LimitPlan, LoadCSV => LoadCSVPlan, Skip => SkipPlan, _}
import com.mware.ge.cypher.internal.util.AssertionRunner.Thunk
import com.mware.ge.cypher.internal.util.Foldable.FoldableAny
import com.mware.ge.cypher.internal.util.attribution.{Attributes, IdGen}
import com.mware.ge.cypher.internal.util.{AssertionRunner, ExhaustiveShortestPathForbiddenException, InternalException}
...
case class LogicalPlanProducer(cardinalityModel: CardinalityModel, planningAttributes: PlanningAttributes, idGen: IdGen) extends ListSupport {
  ...
  def planNodeIndexSeek(idName: String,
    ...
    val cardinality = cardinalityModel(solvedForCardinalityEstimation, context.input, context.semanticTable, context.planContext)
  ...
  def updateSolvedForOr(orPlan: LogicalPlan, orPredicate: Ors, predicates: Set[Expression], context: LogicalPlanningContext): LogicalPlan = {
    ...
    val cardinality = context.cardinality.apply(solved, context.input, context.semanticTable, context.planContext)
  ...
  private def annotate(plan: LogicalPlan, solved: PlannerQuery, providedOrder: ProvidedOrder, context: LogicalPlanningContext) = {
    ...
    val cardinality = cardinalityModel(solved, context.input, context.semanticTable, context.planContext)
    solveds.set(plan.id, solved)
    cardinalities.set(plan.id, cardinality)
    providedOrders.set(plan.id, providedOrder)
    plan
