...
import com.mware.ge.cypher.internal.compiler.planner.StubbedLogicalPlanningConfiguration
import com.mware.ge.cypher.internal.compiler.planner.logical.Metrics.QueryGraphSolverInput
import com.mware.ge.cypher.internal.ir.RegularPlannerQuery
...
class LeafPlanningIntegrationTest extends CypherFunSuite with LogicalPlanningTestSupport2 with AstConstructionTestSupport {
    ...
    (new given {
      ...
      ._2 should equal(
        Selection(Ands(Set(StartsWith(Property(Variable("a") _, PropertyKeyName("name") _) _, StringLiteral("short") _) _))_,
          IndexSeek("a:Person(lastname STARTS WITH 'longer')", propIds = Map("lastname" -> "lastname"))
        ))
  }
  ...
  test("should plan property equality index seek using IN instead of index seek by prefix") {
    ...
    (new given {
      ...
      Selection(Ands(Set(startsWith)) _,
                NodeIndexSeek(
                  "a",
                  LabelToken("Person", LabelId("Person")),
                  Seq(IndexedProperty(PropertyKeyToken(PropertyKeyName("name") _, PropertyKeyId("name")), GetValue)),
  ...
  test("should plan index seek by numeric range for numeric chained operator") {
    ...
    (new given {
      ...
      NodeIndexSeek(
        "a",
        LabelToken("Person", LabelId("Person")),
        Seq(IndexedProperty(PropertyKeyToken(PropertyKeyName("age") _, PropertyKeyId("age")), DoNotGetValue)),
  ...
  test("should plan unique index seek when there is an unique index on the property") {
    ...
    plan._2 should equal(
      NodeUniqueIndexSeek("n", LabelToken("Awesome", LabelId("Awesome")), Seq(IndexedProperty(PropertyKeyToken("prop", PropertyKeyId("prop")), DoNotGetValue)), SingleQueryExpression(SignedDecimalIntegerLiteral("42") _), Set.empty, IndexOrderNone)
    )
  }
  ...
  test("should plan hinted index seek with or") {
    ...
    plan._2 should equal(
      NodeIndexSeek("n", LabelToken("Awesome", LabelId("Awesome")), Seq(IndexedProperty(PropertyKeyToken("prop", PropertyKeyId("prop")), DoNotGetValue)), ManyQueryExpression(ListLiteral(List(SignedDecimalIntegerLiteral("42") _, SignedDecimalIntegerLiteral("1337") _)) _), Set.empty, IndexOrderNone)
    )
  }
  ...
  test("should plan hinted index seek when there are multiple indices") {
    ...
    plan._2 should equal(
      Selection(
        ...
        IndexSeek("n:Awesome(prop2 = 3)", propIds = Map("prop2" -> "prop2"))
      )
    )
  }
  ...
  test("should plan hinted index seek when there are multiple or indices") {
    ...
    val seek2 = IndexSeek("n:Awesome(prop2 = 3)", CanGetValue, propIds = Map("prop2" -> "prop2"))
  ...
  test("should plan hinted unique index seek when there are multiple unique indices") {
    ...
    plan._2 should equal(
      Selection(
        Ands(Set(In(Property(varFor("n"), PropertyKeyName("prop1")_)_, ListLiteral(Seq(SignedDecimalIntegerLiteral("42")_))_)_))_,
        NodeUniqueIndexSeek("n", LabelToken("Awesome", LabelId("Awesome")), Seq(IndexedProperty(PropertyKeyToken("prop2", PropertyKeyId("prop2")), DoNotGetValue)), SingleQueryExpression(SignedDecimalIntegerLiteral("3") _), Set.empty, IndexOrderNone)
      )
    )
  }
  ...
  test("should be able to OR together two index seeks") {
    ...
    val seek1 = IndexSeek("n:Awesome(prop1 = 42)", CanGetValue)
    val seek2 = IndexSeek("n:Awesome(prop2 = 'apa')", CanGetValue, propIds = Map("prop2" -> "prop2"))
    val union: Union = Union(seek2, seek1)
    val distinct = Distinct(union, Map("n" -> varFor("n")))
  ...
  test("should be able to OR together two index range seeks") {
    ...
    val seek2 = IndexSeek("n:Awesome(prop2 STARTS WITH 'apa')", propIds = Map("prop2" -> "prop2"))
  ...
  test("should be able to OR together two index seeks with different labels") {
    ...
    val seek2 = IndexSeek("n:Label2(prop2 = 'val')", CanGetValue, propIds = Map("prop2" -> "prop2"), labelId = "Label2")
  ...
  test("should be able to OR together four index seeks") {
    ...
    val seek1 = IndexSeek("n:Label1(prop1 = 'val')", CanGetValue)
    val seek2 = IndexSeek("n:Label1(prop2 = 'val')", CanGetValue, propIds = Map("prop2" -> "prop2"))
    val seek3 = IndexSeek("n:Label2(prop1 = 'val')", CanGetValue, labelId = "Label2")
    val seek4 = IndexSeek("n:Label2(prop2 = 'val')", CanGetValue, labelId = "Label2", propIds = Map("prop2" -> "prop2"))
