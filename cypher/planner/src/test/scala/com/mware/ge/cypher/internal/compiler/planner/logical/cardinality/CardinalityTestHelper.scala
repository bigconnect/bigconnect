...
import com.mware.ge.cypher.internal.compiler.planner.logical.QueryGraphProducer
...
import com.mware.ge.cypher.internal.util.Cardinality.NumericCardinality
...
trait CardinalityTestHelper extends QueryGraphProducer with CardinalityCustomMatchers {
  ...
  case class TestUnit(query: String,
    ...
    def prepareTestContext:(GraphStatistics, SemanticTable) = {
      val labelIds: Map[String, String] = knownLabelCardinality.map(_._1).zip(knownLabelCardinality.map(_._1)).toMap
      val propertyIds: Map[String, String] = knownProperties.zip(knownProperties).toMap
      val relTypeIds: Map[String, String] = knownRelationshipCardinality.map(_._1._2).toSeq.distinct
        .zip(knownRelationshipCardinality.map(_._1._2).toSeq.distinct).toMap
    ...
    private def fill[T](destination: mutable.Map[String, T], source: Iterable[(String, String)], f: String => T) {
      source.foreach {
