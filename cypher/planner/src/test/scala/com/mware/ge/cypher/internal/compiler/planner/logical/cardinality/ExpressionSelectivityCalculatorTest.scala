 ...
 */
package com.mware.ge.cypher.internal.compiler.planner.logical.cardinality
...
import com.mware.ge.cypher.internal.planner.spi.GraphStatistics.DEFAULT_EQUALITY_SELECTIVITY
import com.mware.ge.cypher.internal.planner.spi.GraphStatistics.DEFAULT_LIST_CARDINALITY
import com.mware.ge.cypher.internal.planner.spi.GraphStatistics.DEFAULT_PROPERTY_SELECTIVITY
import com.mware.ge.cypher.internal.planner.spi.GraphStatistics.DEFAULT_RANGE_SEEK_FACTOR
import com.mware.ge.cypher.internal.planner.spi.GraphStatistics.DEFAULT_RANGE_SELECTIVITY
import com.mware.ge.cypher.internal.planner.spi.GraphStatistics.DEFAULT_STRING_LENGTH
import com.mware.ge.cypher.internal.planner.spi.GraphStatistics.DEFAULT_TYPE_SELECTIVITY
...
import com.mware.ge.cypher.internal.expressions.functions.Distance
...
class ExpressionSelectivityCalculatorTest extends CypherFunSuite with AstConstructionTestSupport {
  ...
  private val indexPerson = IndexDescriptor(LabelId("Person"), Seq(PropertyKeyId("prop")))
  private val indexAnimal = IndexDescriptor(LabelId("Animal"), Seq(PropertyKeyId("prop")))
  ...
  test("Should peek inside sub predicates") {
    ...
    semanticTable.resolvedLabelNames.put("Page", LabelId("Page"))
    ...
    when(stats.nodesWithLabelCardinality(Some(indexPerson.label), null)).thenReturn(1000.0)
  ...
  private def mockStats(allNodesCardinality: Double = 10000.0,
    ...
    labelCardinalities.foreach { case (label, number) =>
      when(stats.nodesWithLabelCardinality(Some(label), null)).thenReturn(number)
    }

    when(stats.indexPropertyExistsSelectivity(any(), null)).thenAnswer(new Answer[Option[Selectivity]] {
    ...
    when(stats.uniqueValueSelectivity(any(), null)).thenAnswer(new Answer[Option[Selectivity]] {
      override def answer(invocationOnMock: InvocationOnMock): Option[Selectivity] = {
        val theIndex = invocationOnMock.getArgument[IndexDescriptor](0)
        for {
