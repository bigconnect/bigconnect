...
import com.mware.ge.cypher.internal.compiler.planner.logical.cardinality.IndependenceCombiner
...
import com.mware.ge.cypher.internal.ir.{VarPatternLength, _}
import com.mware.ge.cypher.internal.planner.spi.GraphStatistics
import com.mware.ge.cypher.internal.util.{Cardinality, LabelId, Selectivity}
import com.mware.ge.cypher.internal.expressions.{HasLabels, LabelName, SemanticDirection}
...
class PatternSelectivityCalculatorTest extends CypherFunSuite  with AstConstructionTestSupport {
  ...
  test("should return zero if there are no nodes with the given labels") {
    ...
    when(stats.nodesWithLabelCardinality(any(), null)).thenReturn(Cardinality(0))
    when(stats.nodesAllCardinality()).thenReturn(Cardinality.EMPTY)
    when(stats.cardinalityByLabelsAndRelationshipType(any(), any(), any(), null)).thenReturn(Cardinality(42))
    ...
    implicit val semanticTable = new SemanticTable(resolvedLabelNames = mutable.Map("L" -> LabelId("L")))
    implicit val selections = Selections(Set(Predicate(Set[String]("a"), HasLabels(varFor("a"), Seq(label))(pos))))
    implicit val planContext = null
  ...
  test("should not consider label selectivity twice") {
    val stats: GraphStatistics = mock[GraphStatistics]
    when(stats.nodesWithLabelCardinality(any(), null)).thenReturn(Cardinality(1))
  ...
  test("handles variable length paths over 32 in length") {
    ...
    when(stats.nodesWithLabelCardinality(any(), null)).thenReturn(Cardinality(1))
    when(stats.nodesAllCardinality()).thenReturn(Cardinality.SINGLE)
    when(stats.cardinalityByLabelsAndRelationshipType(any(), any(), any(), null)).thenReturn(Cardinality(3))
  ...
  test("should not produce selectivities larger than 1.0") {
    ...
    when(stats.nodesWithLabelCardinality(any(), null)).thenAnswer(new Answer[Cardinality] {
    ...
    for (i <- 1 to 100) labels.put(i.toString, LabelId(i.toString))
