...
import com.mware.ge.cypher.internal.compiler.planner.logical.Metrics.{CardinalityModel, QueryGraphSolverInput}
...
class SelectPatternPredicatesTest extends CypherFunSuite with LogicalPlanningTestSupport {
  ...
  doReturn(new CardinalityModel {
    override def apply(query: PlannerQuery, input: QueryGraphSolverInput, semanticTable: SemanticTable, planContext: PlanContext): Cardinality = {
      query match {
