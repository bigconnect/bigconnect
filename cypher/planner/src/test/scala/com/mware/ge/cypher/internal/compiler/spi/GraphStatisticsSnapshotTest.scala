 ...
 */
package com.mware.ge.cypher.internal.compiler.spi
...
class GraphStatisticsSnapshotTest extends CypherFunSuite {
  ...
  test("records queries and its observed values") {
    ...
    instrumentedStatistics.uniqueValueSelectivity(index, null)
    instrumentedStatistics.nodesWithLabelCardinality(Some(label4), null)
    instrumentedStatistics.cardinalityByLabelsAndRelationshipType(Some(label2), None, None, null)

    snapshot.freeze.statsValues should equal(Map(
      NodesAllCardinality -> allNodes,
      IndexSelectivity(index, null) -> indexSelectivity,
      NodesWithLabelCardinality(Some(label4), null) -> nodesWithLabel,
      CardinalityByLabelsAndRelationshipType(Some(label2), None, None, null) -> relationships
    ))
  }
  ...
  test("if threshold is 1.0 nothing diverges") {
    ...
    val instrumentedStatistics1 = InstrumentedGraphStatistics(statistics, snapshot1)
    instrumentedStatistics1.nodesAllCardinality()
    instrumentedStatistics1.uniqueValueSelectivity(index, null)
    instrumentedStatistics1.nodesWithLabelCardinality(Some(label4), null)
  ...
  private def graphStatistics(allNodes: Long = 500,
    ...
    def nodesWithLabelCardinality(labelId: Option[LabelId], planContext: PlanContext): Cardinality = labelId match {
