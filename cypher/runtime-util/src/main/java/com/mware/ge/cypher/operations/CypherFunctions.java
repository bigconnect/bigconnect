...
import com.mware.ge.cypher.internal.util.ParameterWrongTypeException;
...
public final class CypherFunctions {
    ...
    public static Value distance(AnyValue lhs, AnyValue rhs) {
        if (lhs instanceof GeoPointValue && rhs instanceof GeoPointValue) {
            return calculateDistance((GeoPointValue) lhs, (GeoPointValue) rhs);
        } else {
            return NO_VALUE;
        }
    }
    ...
    public static AnyValue propertyGet(String key, AnyValue container, DbAccess dbAccess) {
        if (container instanceof VirtualNodeValue) {
            return dbAccess.nodeProperty(((VirtualNodeValue) container).id(), key);
        } else if (container instanceof VirtualRelationshipValue) {
            return dbAccess
                    .relationshipProperty(((VirtualRelationshipValue) container).id(), key);
        } else if (container instanceof MapValue) {
    ...
    public static TextValue id(AnyValue item) {
        if (item instanceof VirtualNodeValue) {
            return stringValue(((VirtualNodeValue) item).id());
        } else if (item instanceof VirtualRelationshipValue) {
            return stringValue(((VirtualRelationshipValue) item).id());
        } else {
    ...
    public static boolean hasLabel(AnyValue entity, String label, DbAccess access) {
        if (entity instanceof NodeValue) {
            return access.isLabelSetOnNode(label, ((NodeValue) entity).id());
        } else {
    ...
    public static Value point(AnyValue in, DbAccess access) {
        if (in instanceof VirtualNodeValue) {
            ...
            return GeoPointValue.fromMap(map);
        } else {
    ...
    public static TextValue toString(AnyValue in) {
        ...
        } else if (in instanceof TemporalValue || in instanceof DurationValue) {
    ...
    private static ListValue extractKeys(DbAccess access, String[] keyIds) {
        return VirtualValues.fromArray(Values.stringArray(keyIds));
    }
    ...
    private static Value asPoint(DbAccess access, VirtualNodeValue nodeValue) {
        ...
        return GeoPointValue.fromMap(builder.build());
    }
    ...
    private static Value calculateDistance(GeoPointValue p1, GeoPointValue p2) {
