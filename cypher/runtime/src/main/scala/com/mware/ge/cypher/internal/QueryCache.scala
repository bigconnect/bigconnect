...
import com.mware.ge.cypher.internal.compatibility.CypherCacheMonitor
...
class QueryCache[QUERY_REP <: AnyRef, QUERY_KEY <: Pair[QUERY_REP, ParameterTypeMap], EXECUTABLE_QUERY <: AnyRef](
  ...
  /**
    ...
    * @param qc GeCypherQueryContext in which to compile and compute staleness
  ...
  def computeIfAbsentOrStale(queryKey: QUERY_KEY,
    ...
    if (maximumSize == 0)
      ...
      inner.getIfPresent(queryKey) match {
        case NOT_PRESENT =>
          compileAndCache(queryKey, qc, compile, metaData)

        case cachedValue =>
          //mark as seen from cache
          cachedValue.markHit()

          stalenessCaller.staleness(qc, cachedValue.value) match {
            case NotStale =>
  ...
  private def compileAndCache(queryKey: QUERY_KEY,
                              geCypherQueryContext: GeCypherQueryContext,
                              compile: () => EXECUTABLE_QUERY,
                              metaData: String): CacheLookup[EXECUTABLE_QUERY] = {
