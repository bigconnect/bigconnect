...
import com.mware.ge.cypher.exception.QueryExecutionException
import com.mware.ge.cypher.internal.compatibility.runtime.executionplan.{DelegatingExecutionPlan, ExecutionPlan}
...
import com.mware.ge.cypher.internal.compiler.{CypherPlannerConfiguration, RuntimeUnsupportedNotification}
...
import com.mware.ge.cypher.{CypherRuntimeOption, InvalidArgumentException, exceptionHandler}
  ...
  private def publicCannotCompile(originalException: Exception) =
    {
      ...
      new QueryExecutionException(message, invalidArgument, invalidArgument.status.code().serialize())
    }
