...
import com.mware.ge.cypher.internal.runtime.interpreted.pipes.{OwningPipeAsserter, Pipe, PipeBuilderFactory, PipeExecutionBuilderContext}
import com.mware.ge.cypher.internal.logical.plans.{LogicalPlan, LogicalPlans}
...
class PipeExecutionPlanBuilder(pipeBuilderFactory: PipeBuilderFactory,
  ...
  val LOG = BcLoggerFactory.getLogger(classOf[PipeExecutionPlanBuilder])
  ...
  def build(plan: LogicalPlan)
    ...
    try {
      ...
      case e:InternalException => LOG.error(e.getMessage)
    }
    pipe
  }
