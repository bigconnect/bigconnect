...
import com.mware.ge.cypher.Result.ResultRow

import scala.collection.Map

class MapBasedRow(var map: Map[String, Any] = Map.empty) extends ResultRow {

  override def getNode(key: String): Vertex = getWithType(key, classOf[Vertex])

  override def getRelationship(key: String): Edge = getWithType(key, classOf[Edge])
