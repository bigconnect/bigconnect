...
import com.mware.ge.values.{AnyValue, AnyValueWriter}
...
object CastSupport {
  ...
  private def transform(writer: ArrayConverterWriter)(value: ListValue): ArrayValue = {
    value.writeTo(writer, new Authorizations())
    writer.array
  }
  ...
  private class ArrayConverterWriter(typ: Class[_], transformer: (AnyRef) => ArrayValue)
    ...
    override def writeNodeReference(nodeId: String): Unit = fail()

    override def writeNode(nodeId: String, labels: TextArray,
                           properties: MapValue,
                           authorizations: Authorizations): Unit = fail()

    override def writeRelationshipReference(relId: String): Unit = fail()

    override def writeRelationship(relId: String, startNodeId: String, endNodeId: String, `type`: TextValue,
                                   properties: MapValue, authorizations: Authorizations): Unit = fail()
    ...
    override def writePath(nodes: Array[NodeValue],
                           relationships: Array[RelationshipValue], authorizations: Authorizations): Unit = fail()
    ...
    override def writeGeoPoint(geoPoint: GeoPoint): Unit = {
      write(geoPoint)
    }
