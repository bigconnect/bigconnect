...
import com.mware.ge.cypher.internal.runtime.EntityById
...
trait ExecutionContext extends MutableMap[String, AnyValue] {
  ...
  /**
    ...
    */
  def invalidateCachedProperties(node: String): Unit
  ...
  def boundEntities(materializeNode: String => AnyValue, materializeRelationship: String => AnyValue): Map[String, AnyValue]
...
class MapExecutionContext(private val m: MutableMap[String, AnyValue], private var cachedProperties: MutableMap[CachedNodeProperty, Value] = null)
  ...
  override def boundEntities(materializeNode: String => AnyValue, materializeRelationship: String => AnyValue): Map[String, AnyValue] =
    m.collect {
  ...
  override def invalidateCachedProperties(node: String): Unit = {
    if (cachedProperties != null)
      cachedProperties.keys.filter(cnp => apply(cnp.nodeVariableName) match {
        case n: VirtualNodeValue => n.id().equals(node)
        case _ => false
