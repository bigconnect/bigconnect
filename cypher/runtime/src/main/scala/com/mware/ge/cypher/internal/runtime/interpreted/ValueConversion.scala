...
import com.mware.ge.{Edge, Vertex}
import com.mware.ge.cypher.internal.util.symbols._
import com.mware.ge.cypher.values.utils.ValueUtils
import com.mware.ge.values.storable.Values.byteArray
import com.mware.ge.values.storable._
import com.mware.ge.values.virtual.VirtualValues.fromArray
import com.mware.ge.values.virtual.{MapValue, MapValueBuilder, VirtualValues}

object ValueConversion {
  def getValueConverter(cType: CypherType): Any => AnyValue = {
    val converter: Any => AnyValue = cType match {
      case CTNode => n => ValueUtils.fromNodeProxy(n.asInstanceOf[Vertex])
      case CTRelationship => r => ValueUtils.fromRelationshipProxy(r.asInstanceOf[Edge])
  ...
  def asValue(value: Any): AnyValue = value match {
    ...
    case n: Vertex => ValueUtils.fromNodeProxy(n)
    case r: Edge => ValueUtils.fromRelationshipProxy(r)
