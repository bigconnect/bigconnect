...
import com.mware.ge.cypher.internal.runtime.interpreted.IsList
...
import com.mware.ge.cypher.internal.runtime.interpreted.commands.expressions.AbstractCachedNodeProperty
import com.mware.ge.cypher.operations.CypherBoolean
...
case class HasLabel(entity: Expression, label: KeyToken) extends Predicate {
  ...
  override def isMatch(m: ExecutionContext, state: QueryState): Option[Boolean] = entity(m, state) match {
    ...
    case value =>
      ...
      Some(queryCtx.isLabelSetOnNode(label.name, nodeId))
  }
