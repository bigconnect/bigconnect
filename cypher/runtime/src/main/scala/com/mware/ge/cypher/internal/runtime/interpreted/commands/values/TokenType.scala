...
trait TokenType  {
  ...
  def apply(name: String, id: String) = KeyToken.Resolved(name, id, this)

  def getOptIdForName(name: String, tokenContext: TokenContext): Option[String]

  def getIdForNameOrFail(name: String, tokenContext: TokenContext): String

  def getOrCreateIdForName(name: String, queryContext: QueryContext): String
}
