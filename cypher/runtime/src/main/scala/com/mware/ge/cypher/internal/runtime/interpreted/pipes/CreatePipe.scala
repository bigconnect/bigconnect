...
abstract class BaseCreatePipe(src: Pipe) extends PipeWithSource(src) {
  ...
  /**
    ...
    */
  protected def setProperty(entityId: String,
                            key: String,
                            value: AnyValue,
                            qtx: QueryContext,
                            ops: Operations[_]): Unit = {
    ...
    if (value == Values.NO_VALUE) {
      ...
      ops.setProperty(entityId, key, makeValueNeoSafe(value))
    }
  }
...
abstract class EntityCreatePipe(src: Pipe) extends BaseCreatePipe(src) {
  ...
  protected def createNode(context: ExecutionContext,
    ...
    val node = state.query.createNode(labelIds, idProp(context, state, data.properties))
  ...
  protected def createRelationship(context: ExecutionContext,
    ...
    val relationship =
      if (start == null || end == null)
        ...
        val relationship = state.query.createRelationship(start.id(), end.id(), typeId, idProp(context, state, data.properties))
  ...
  private def idProp(context: ExecutionContext,
                     state: QueryState,
                     properties: Option[Expression]): Option[AnyValue] = {
    properties.map(p => {
      p(context, state) match {
        case IsMap(map) =>
          map(state.query).get("__id")
        case _ =>
          return null
      }
    })
