...
import com.mware.ge.cypher.internal.runtime.interpreted.commands.expressions.NumericHelper
...
abstract class IdSeekIterator[T]
  ...
  private def computeNextEntity(): T = {
    while (entityIds.hasNext) {
      val maybeEntity = for {
        id <- asStringEntityId(entityIds.next())
  ...
  protected def asStringEntityId(a: AnyValue): Option[String] = a match {
    case a: TextValue => Some(a.stringValue())
    case _ => None
  }
}
