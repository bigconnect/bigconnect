...
abstract class NodeOuterHashJoinPipe(nodeVariables: Set[String],
  ...
  protected def computeKey(context: ExecutionContext): Option[IndexedSeq[String]] = {
    val key = new Array[String](myVariables.length)
...
class ProbeTable() {
  private val table: mutable.HashMap[IndexedSeq[String], MutableList[ExecutionContext]] =
    new mutable.HashMap[IndexedSeq[String], MutableList[ExecutionContext]]

  private val rowsWithNullInKey: ListBuffer[ExecutionContext] = new ListBuffer[ExecutionContext]()

  def addValue(key: IndexedSeq[String], newValue: ExecutionContext) {
  ...
  def apply(key: IndexedSeq[String]): MutableList[ExecutionContext] = table.getOrElse(key, EMPTY)

  def keySet: collection.Set[IndexedSeq[String]] = table.keySet
