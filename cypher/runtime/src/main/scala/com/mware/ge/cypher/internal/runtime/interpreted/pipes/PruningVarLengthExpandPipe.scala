...
import com.mware.ge.values.virtual.{RelationshipValue, VirtualNodeValue}
...
case class PruningVarLengthExpandPipe(source: Pipe,
  ...
  class PruningDFS(val state: FullPruneState,
                   ...
                   val expandMap: util.HashMap[String, NodeState],
    ...
    private def seenRelationshipInPath(r: String): Boolean = {
  ...
  class NodeState() {
    ...
    /**
      ...
      */
    def minOutgoingDepth(incomingRelId: String): Int = {
  ...
  class FullPruneState(queryState:QueryState ) {
    ...
    private val path = new Array[String](max)
    private var depth = -1
        ...
        if (depth == -1) {
          ...
          fromValue match {
            case node: VirtualNodeValue =>
              push( node = node,
                pathLength = 0,
                expandMap = new util.HashMap[String, NodeState](),
                prevLocalRelIndex = -1,
    ...
    def push(node: VirtualNodeValue,
             pathLength: Int,
             expandMap: util.HashMap[String, NodeState],
