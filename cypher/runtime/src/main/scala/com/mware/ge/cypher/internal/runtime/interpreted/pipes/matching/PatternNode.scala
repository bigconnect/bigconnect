...
import com.mware.ge.cypher.internal.runtime.interpreted.commands.values.{KeyToken, UnresolvedProperty}
import com.mware.ge.cypher.internal.runtime.interpreted.commands.{RelatedTo, SingleNode}
...
class PatternNode(key: String, val labels: Seq[KeyToken] = Seq.empty, val properties: Map[KeyToken, Expression] = Map.empty)
  ...
  def canUseThis(graphNodeId: String, state: QueryState, ctx: ExecutionContext): Boolean =
  ...
  private def nodeHasLabels(graphNodeId: String, ctx: QueryContext): Boolean = {
    val expectedLabels: Seq[Option[String]] = labels.map(_.getOptName())
  ...
  private def nodeHasProperties(graphNodeId: String, execCtx: ExecutionContext, state: QueryState): Boolean =
    properties.forall {
