...
abstract class BaseFeatureTest {
  ...
  def filterScenarios(allScenarios: Seq[Scenario], featureToRun: String, scenarioToRun: String): Seq[BcScenario] = {
    val bcScenarios = allScenarios.map(s => BcScenario(s))

    if (featureToRun.nonEmpty) {
      val filteredFeature = bcScenarios.filter(bs => bs.featureName.contains(featureToRun))
      if (scenarioToRun.nonEmpty) {
        filteredFeature.filter(bs => bs.name.equals(scenarioToRun))
      } else
        filteredFeature
    } else if (scenarioToRun.nonEmpty) {
      bcScenarios.filter(s => s.name.equals(scenarioToRun))
    } else
      bcScenarios
  }
}
