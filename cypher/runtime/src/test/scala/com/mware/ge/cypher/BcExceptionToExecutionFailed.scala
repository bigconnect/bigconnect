...
object BcExceptionToExecutionFailed {

  def convert(phase: String, t: Throwable): ExecutionFailed = {
    val cypherException = t match {
      case re: RuntimeException => re
      case _ => throw t
    }
    val errorType = Status.statusCodeOf(cypherException)
    val msg = cypherException.getMessage
  ...
  private def runtimeDetail(msg: String): String = {
    ...
    else if (msg.matches("((Node)|(Relationship)) with id \\d+ was not found"))
