...
import com.mware.ge.cypher.procedure.{Mode, impl}
import com.mware.ge.io.ResourceTracker
import com.mware.ge.cypher.internal.util.symbols.{CTBoolean, CTFloat, CTInteger, CTMap, CTNode, CTNumber, CTPath, CTRelationship, CTString, CypherType, ListType}
import com.mware.ge.cypher.procedure.impl.CallableProcedure.BasicProcedure
import com.mware.ge.cypher.procedure.impl.{Context, Neo4jTypes}
...
case class BcValueRecords(header: List[String], rows: List[Array[AnyRef]])

object BcValueRecords {
  def apply(record: CypherValueRecords): BcValueRecords = {
    val tableValues = record.rows.map {
      row: Map[String, CypherValue] =>
        record.header.map { columnName =>
          val value = row(columnName)
          val convertedValue = TCKValueToBcValue(value)
          convertedValue
        }.toArray
    }
    BcValueRecords(record.header, tableValues)
  }
}

trait BcProcedureAdapter extends ProcedureSupport {
  self: Graph =>

  protected def executionEngine: GeCypherExecutionEngine
  ...
  override def registerProcedure(signature: String, values: CypherValueRecords): Unit = {
    ...
    Try(executionEngine.registerProcedure(kernelProcedure)) match {
      case Success(_) =>
  ...
  private def buildProcedure(parsedSignature: ProcedureSignature, values: CypherValueRecords) = {
    ...
    val neo4jValues = BcValueRecords(values)
  ...
  private def asKernelSignature(parsedSignature: ProcedureSignature): impl.ProcedureSignature = {
    val builder = impl.ProcedureSignature.procedureSignature(parsedSignature.namespace.toArray, parsedSignature.name)
    ...
    parsedSignature.outputs match {
      ...
      case None => builder.out(impl.ProcedureSignature.VOID)
    }
    builder.build()
  }
