...
object ScenarioTestHelper {
  def createTests(scenarios: Seq[BcScenario],
                  config: TestConfig,
                  bcEngine: CypherQueryContextFactory,
                  debugOutput: Boolean = false): java.util.Collection[DynamicTest] = {
    ...
    val expectFailTests: Seq[DynamicTest] = expectFail.map { scenario =>
      ...
      val executable = new Executable {
        override def execute(): Unit = {
          Try {
            scenario(BcTestAdapter(config.executionPrefix, bcEngine)).execute()
          } match {
    ...
    val expectPassTests: Seq[DynamicTest] = expectPass.map { scenario =>
      val name = scenario.toString()
      val executable = scenario(BcTestAdapter(config.executionPrefix, bcEngine))
      DynamicTest.dynamicTest(name, executable)
    }
    (expectPassTests ++ expectFailTests).asJavaCollection
  }

  def checkForDuplicates(scenarios: Seq[BcScenario], blacklist: Seq[BlacklistEntry]): Unit = {
    // test scenarios
    var testScenarios = new mutable.HashSet[BcScenario]()
    for (s <- scenarios) {
   ...
   */
  def printComputedBlacklist(scenarios: Seq[BcScenario], config: TestConfig, bcEngine: CypherQueryContextFactory): Unit = {
    ...
    val blacklist = scenarios.zipWithIndex.flatMap { case (scenario, index) =>
      val isFailure = Try(scenario(BcTestAdapter(config.executionPrefix, bcEngine)).execute()).isFailure
