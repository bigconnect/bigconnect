...
public abstract class KeyBase {
        ...
        while (true) {
            if (i > 0) {
                ...
                if (partIndex >= partCount) {
                    throw new GeAccumuloIteratorException("Invalid number of parts for '" + s + "'. Expected " + partCount + " found " + partIndex);
                }
                last = i + 1;
                i = s.indexOf(VALUE_SEPARATOR, last);
            } else {
    ...
    public static void assertNoValueSeparator(String str) {
        if (str.indexOf(VALUE_SEPARATOR) >= 0) {
            throw new GeInvalidKeyException("String cannot contain '" + VALUE_SEPARATOR + "' (0x1f): " + str);
        }
    }
}
