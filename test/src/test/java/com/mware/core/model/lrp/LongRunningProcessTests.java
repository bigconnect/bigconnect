...
public class LongRunningProcessTests extends TestBaseWithInjector {
    LongRunningProcessRepository longRunningProcessRepository;

    @Before
    public void before() {
        getConfigMap().put("status.enabled", true);
        getConfigMap().put(LongRunningProcessRunnerProcess.class.getName() + ".threadCount", 1);

        super.before();

        InjectHelper.getInstance(LongRunningProcessRunnerProcess.class);
        longRunningProcessRepository = InjectHelper.getInstance(LongRunningProcessRepository.class);
    }
